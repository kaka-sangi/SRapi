...
> 本文档描述的是 **已落地并在生产运行的架构**（0.1.0 发布版）。文中的核心不变量（Provider-neutral 调度内核、OpenAPI 唯一契约、Redis 仅承载可重建状态、凭证不明文、跨模块只走 contract / 领域事件）均由 `internal/architecture` 的 architecture-check 测试持续校验。历史阶段性表述已按当前实现重述；仍属规划范围的内容会显式标注 “Roadmap / 尚未实现”。

## 1. 架构目标

SRapi 后端采用模块化单体架构，目标是避免微服务复杂度，同时通过清晰的模块边界、接口抽象和依赖方向，为持续扩展 Provider、模型能力、调度策略、计费方式和部署形态预留空间。
...
可执行架构要求与 CI gate（harness）见 `ARCHITECTURE_REQUIREMENTS.md`。
...
- 单进程部署，作为一个有意的产品取舍：在不引入微服务运维复杂度的前提下保持清晰边界。
- 内部按领域模块拆分。
- 模块之间通过接口交互。
...
│       └── main.go
├── internal/
│   ├── app/                # 应用组装与生命周期（app.go）
│   ├── architecture/       # architecture-check：依赖方向与边界守卫测试
│   ├── codequality/        # 代码体量/复杂度等质量 gate 测试
│   ├── config/             # 环境变量加载与校验
│   ├── httpserver/         # HTTP 协议层：路由、middleware、OpenAPI handler adapter、统一响应
│   ├── modules/            # 业务模块（约 40 个，见 §7）
│   ├── openapi/            # OpenAPI 生成代码（openapi.gen.go，不得手改）
│   ├── persistence/        # 持久化实现
│   │   ├── entstore/        #   Ent 持久化 store（含 events outbox/inbox）
│   │   └── redisstore/      #   Redis-backed store（scheduler / realtime / sessionaffinity）
│   ├── platform/           # 基础设施抽象
│   │   ├── crypto/
│   │   ├── db/
│   │   ├── glob/
...
│   │   ├── ratelimit/
│   │   └── redis/
│   ├── testsupport/        # 测试辅助（oteltest 等）
│   └── workers/            # 后台轮询/异步投递任务（outbox、quota_refresh、order_expirer 等，见 §4.7）
├── ent/
...
对象存储（`STORAGE_BACKEND` 等）能力当前通过 `internal/config` + `internal/app` 装配，并在 `internal/httpserver` 运行时使用，未单列为独立 platform 包；HTTP client / 时间源等依旧由对应模块按需注入。
...
基础设施能力。已落地的 platform 包：

- `crypto`：凭证加解密 / 密钥。
- `db`：数据库连接与迁移。
- `redis`：Redis 连接。
- `logger`：结构化日志与 context 透传。
- `otel`：trace / span。
- `ratelimit`：限流脚本与令牌桶。
- `glob`：模式匹配辅助。

HTTP client、时间源、对象存储等能力按需在模块或装配层注入，未单列为独立 platform 包。
...
承载后台轮询和异步投递任务。当前已落地的 worker 包括 `outbox`（领域事件投递）、`quota_refresh`、`order_expirer`、`subscription_expirer`、`balance_charger`、`connectivity_test`、`health_probe`、`slo_evaluator`、`account_quota_alert`、`auth_session_cleanup`、`idempotency_cleanup`、`retention`、`scheduled_test`、`quality_eval` 等（以 `apps/api/internal/workers` 为准）。
...
下文逐一说明的是核心业务模块的职责边界。`internal/modules` 实际包含约 40 个模块，除下列核心模块外，还包括 `users`、`totp`、`captcha`、`copilot`、`notifications`、`realtime`、`reverse_proxy`、`content_safety`、`error_passthrough`、`tls_profiles`、`user_platform_quotas`、`payload_rules`、`model_rate_limits`、`group_rate_limits`、`channel_monitors`、`scheduled_tests`、`quality_eval`、`operations`、`health_rollups`、`userattributes`、`admin_control`、`account_provisioning`、`capabilities`、`provider_adapters`、`idempotency`、`events`、`usage`、`sessionaffinity` 等。完整清单以 `apps/api/internal/modules` 目录为准；新增模块同样遵循下文及 §5 的依赖方向与 contract 约束。
...
用户侧 affiliate 查询只读取当前登录用户的账本 summary/ledger；转余额走 Affiliate service 的幂等 `TransferToBalance` 命令，并在持久化 store 中同事务写 affiliate ledger、billing ledger 和 user balance。
...
Domain Events Outbox 使用数据库轮询 dispatcher（`internal/workers/outbox` + `internal/persistence/entstore/events`）：`pending` 事件成功后进入 `published`，失败后记录 `attempt_count`、`last_error` 和 `next_retry_at`，到期后可重试。这是当前在用的投递机制；若未来替换为外部消息中间件（Roadmap），不得改变事件 envelope 和 inbox 幂等语义。
...
> 以下为项目早期（第一阶段）设定的架构落地基线，**均已达成并被后续约 150 个工作包远远超越**。保留于此仅作历史背景；当前能力的权威来源是各专项 spec、`apps/api` 代码与 `../../specs/plans/STATUS.md`。

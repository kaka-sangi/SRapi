...
AdminOps 系统日志已经提升为 operations-owned `ops_system_logs` 一等表，
不能再放回 `settings.value_json`，也不能由 admin-control settings 合约
重复维护。
AdminOps 上游错误证据已经提升为 `ops_error_logs` 一等表。它保存网关热路径
观察到的上游失败摘要、关联 ID、处理状态和脱敏正文片段；不能回退为内存状态，
也不能通过 usage 日志临时派生来替代生产排障证据。
...
AdminOps 上游错误证据表。它是运维排障用的结构化错误日志，不是请求重放队列。
...
```txt
index(occurred_at)
index(resolution, occurred_at)
index(error_class, occurred_at)
index(platform, occurred_at)
...
- 不引入 sub2api 式 retry/replay 请求体存储。可回放请求应继续依赖已脱敏的 scheduler request snapshots 或显式 idempotency record；错误日志只负责诊断。
- 生产 runtime 必须注入持久化 store；memory store 只允许用于测试/开发降级。

### 15B.5 obs_alert_rules

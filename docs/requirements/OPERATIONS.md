...
AdminOps `ops_system_logs` 是可查询的低敏运维时间线。Gateway 上游尝试失败、no-available-account 决策以及 usage_log 写入失败必须写入该时间线；原始上游 body、prompt、header、cookie 和凭证不得进入该表，排障详情通过 `request_id` 关联 `ops_error_logs` 和请求转储文件。

...
Gateway 路由族、Provider alias、passthrough 和 WebSocket 阶段边界以 `GATEWAY_ROUTE_MATRIX.md` 为准。`/v1/chat/completions`、`/v1/responses`、`/v1/messages` 以及对应 Provider alias 入口在调度前必须分别要求 `chat_completions`、`responses`、`messages` effective capability，端点协议支持不能只由模型映射或模型 capability 代替；Provider alias 的原始路径只作为 scheduler decision、usage log、realtime slot 与 ops evidence 的 `source_endpoint`，协议归一化和 Provider Adapter 分发仍使用标准 Gateway endpoint；已实现并测试覆盖的 adapter 转换可以声明相应 effective capability，例如 Codex CLI 将 Chat/Messages 入口转换为 Responses 上游，Anthropic/Gemini adapters 将受支持的文本入口转换为各自上游协议。xAI/Grok 使用独立 `xai_compatible` preset family 和 `native-grok` adapter type；其 REST wire protocol 仍是 OpenAI-compatible，但能力基线必须来自 Grok preset，而不是继承通用 `openai-compatible` 子资源。`/v1/images/generations`、`/v1/images/edits`、`/v1/images/variations` 在调度前必须分别要求 `image_generations`、`image_edits`、`image_variations` effective capability；SRapi 不使用 `images` 伞形 endpoint capability。`/v1/images/edits` 和 `/v1/images/variations` 的 OpenAPI contract 同时描述 multipart form-data 和 application/json image reference bodies；JSON references 只允许本地 data URL / base64 payload，edit JSON body 允许源图像和可选 `mask` 使用该引用形状，remote URL 与 `file_id` references 由运行时明确拒绝，variation JSON body 只允许单个源图像，直到 Files API / remote-fetch 安全边界实现。`/v1/images/edits` 的 OpenAI-compatible API-key / 普通 reverse-proxy accounts 调用 multipart `/images/edits`；`/v1/images/variations` 的 OpenAI-compatible API-key / 普通 reverse-proxy accounts 调用 multipart `/images/variations`；`reverse-proxy-codex-cli` accounts 会把 edits 和 variations 都转换为 Codex `/responses` `image_generation` tool `action=edit`，variation 使用源图像作为唯一 input image。`stream=true` 返回 `text/event-stream`；Codex、OpenAI-compatible API-key 和普通 reverse-proxy image generation/edit streams 会把 Responses-style 或原生 image progressive SSE 统一转换为 `image_generation.partial_image` / `image_generation.completed` 并从最终事件解析 usage。`/v1/videos`、`/v1/videos/{video_id}` 和 `/v1/videos/{video_id}/content` 是 OpenAI-style video gateway 路由，调度前要求 `videos` effective capability；create 发送到 xAI/Grok `/videos/generations` 并把返回 video ID 绑定到选中 Provider Account，retrieve/content 必须通过 API Key + video ID 找到该绑定并 hard sticky 到同一账号，缺少绑定返回 `video_binding_not_found`。`/v1/responses/{response_id}/input_items` 是 Responses 子资源读取，调度前要求 `responses_input_items` effective capability；普通 `responses` 生成能力不能自动替代它，因为 Anthropic/Gemini 等跨协议生成转换不能读取 OpenAI Responses stateful input item list。`/v1/responses/compact` 复用 `ResponsesRequest` 和标准 Gateway 证据链，调度前要求 `responses_compact` effective capability；`responses` capability 不能自动替代 `responses_compact`，因为 compact 需要明确的 `response.compaction` JSON 语义；compact 按非流式契约处理，即使客户端发送 `stream=true`，也会返回单个 JSON body，并支持可选 `Idempotency-Key` 重放保护；同协议 Codex/OpenAI/Grok compact 响应按原始 `response.compaction` JSON 回放，不在跨协议路径伪造压缩语义。`/v1/responses/ws` 在 OpenAPI 中以 WebSocket upgrade route 表达；运行时 JSON frame schema 复用 `ResponsesRequest`，不新增 provider-specific Gateway DTO；请求上游 WebSocket relay 时调度前要求 `responses_websocket` effective capability。`/v1/realtime` 是 OpenAI-compatible Realtime WebSocket upgrade route，模型从 query `model` 解析；API-key accounts 走官方 API-key upstream relay，OAuth/session/client-token accounts 走 Reverse Proxy Runtime 双向 relay；不是 `POST /v1/realtime`。`GET /v1beta/models` 与 `GET /v1beta/models/{model}` 是 Gemini-compatible models.list / models.get 形状，只做 Gateway API Key 鉴权、模型可见性过滤和 Google-shaped rendering，不获取 Scheduler lease，也不读取 Provider Account 凭证。`POST /v1beta/models/{model}:generateContent` 与 `:streamGenerateContent` 是 Gemini-compatible 生成形状，调度前要求 `gemini_generate_content` effective capability。`POST /v1beta/models/{model}:countTokens` 是 Gemini-compatible countTokens 形状，走 Gateway API Key、模型可见性、entitlement、Scheduler、Provider Adapter 和 Reverse Proxy Runtime 边界，并要求 `gemini_count_tokens` + `token_counting`；成功请求只记录 Scheduler/request evidence，generation usage tokens 和 cost 保持 0。`POST /v1/messages/count_tokens` 是 Anthropic-compatible count_tokens 形状，要求 `anthropic_count_tokens` + `token_counting`；Gateway 只用归一化结果做 policy / entitlement / Scheduler / evidence，Provider Adapter 保留 Anthropic body 形状并把 `model` 映射成上游模型后调用 `/messages/count_tokens`，成功请求同样不进入生成用量和成本。

`GET /v1/models?client_version=...` 是 Codex CLI 发现模型时使用的查询变体；鉴权、API Key allowed_models、alias 与账号可用性过滤规则与普通 `/v1/models` 完全相同，但成功响应使用 `{"models":[...]}` 的 Codex client catalog 形状，而不是 OpenAI `{"object":"list","data":[...]}`。该变体只渲染当前 Gateway key 可见模型；已知 Codex 模型使用 Codex CLI 模板字段，自定义模型使用保守默认模板并关闭 `prefer_websockets`。

OpenAI Responses-family SSE 一旦已经开始发送，后续 upstream read error 或 idle timeout 不能退化为通用 `event: error`；必须按 Responses lifecycle 以 `event: response.failed` 结束，payload 至少包含 `type=response.failed`、`response.status=failed` 和 `response.error.code/message`。Gateway 仍需在 `ops_error_logs` 记录对应 `error_class`、`error_phase`、`error_source` 与 `stream_completion_state`，用于 AdminOps 归因。
...
`AdminSettingsGateway.retry_count` 是跨候选账号的 Gateway failover 最大尝试次数，不是同账号重试次数；默认值为 20，使 `session_invalid`、`auth_failed`、`rate_limit`、`provider_5xx` 等账号/上游级失败优先继续尝试账号池，管理员仍可显式调低该上限。
...
`GET /v1/models?client_version=...` 复用 `/v1/models` 的模型可见性逻辑，但响应体切换为 Codex CLI client catalog。该查询变体用于 Codex CLI/桌面客户端发现 `base_instructions`、reasoning levels、service tiers 和客户端最低版本等模型元数据。

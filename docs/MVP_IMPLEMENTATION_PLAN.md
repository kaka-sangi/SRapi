...
> **现状说明（事后补注）：** 下列绝大多数项目在 MVP 之后均已交付，**仅 Batch API、Fine-tuning API 和 Kubernetes 部署仍属真正未实现的 Roadmap 项**。

- ~~支付。~~ **已交付**（payments 模块，Stripe / EasyPay / Alipay / WeChat 等多渠道 + 退款 + Webhook）。
- ~~订阅购买。~~ **已交付**（subscriptions 模块 + 订阅计划 / 权益）。
- ~~多支付渠道。~~ **已交付**（见上）。
- ~~复杂 RBAC。~~ **已交付**（Role.permissions + Entitlement schema + 角色编辑/删除及内置角色保护）。
- ~~高级策略 DSL。~~ **部分交付**（Scheduler 策略注册与扩展点已落地；声明式 DSL 仍可视场景演进）。
- 机器学习调度。 **Roadmap / 尚未实现**（当前为可解释的评分调度内核，非 ML 模型驱动）。
- ~~Realtime API。~~ **已交付**（`/v1/realtime` WebSocket 中继，realtime 模块）。
- Batch API。 **Roadmap / 尚未实现**。
- Fine-tuning API。 **Roadmap / 尚未实现**。
- Kubernetes 部署。 **Roadmap / 尚未实现**（当前为单进程 / Docker Compose 自托管部署）。
...
> **现状说明：** 该约束仅适用于 MVP 阶段。当前已交付一套完整的 Next.js 管理 + 自助控制台（apps/web，含 Playground 与 Copilot），视觉设计（暖纸 Concept A）已落地。
...
第一版允许（下列为 MVP 当时刻意接受的技术债，**现已基本偿还**，见各项补注）：

- 简单 RBAC。 *（现已升级为 Role.permissions + Entitlement 体系。）*
- 简单 admin bootstrap。
- 单 Provider Adapter。 *（现已有 10+ 个适配器，含 OpenAI/Anthropic/Gemini 兼容、Codex、Claude Code、Antigravity、Bedrock、reverse-proxy 变体等。）*
- 简单策略权重配置。
- 简单 usage 估算。
- 简单 billing hook，不做完整支付。 *（现已交付完整支付流程：多渠道 + 退款 + Webhook + 账单台账。）*

下列约束**始终**不允许（MVP 至今一直为硬性约束）：
...
> **现状说明：** 下列三个阶段**均已交付**——Billing / Subscription / Payment（支付 + 订阅 + 账单台账 + 返利），Cache Affinity / Cost Optimization（会话粘度 / session affinity + cost_saver 策略），Multi-provider Expansion / Advanced Governance（10+ 适配器 + 权益 / 配额 / 审计 / 可观测治理）。当前真正未实现的 Roadmap 项见 §2 的现状说明（Batch / Fine-tuning / Kubernetes）。

MVP 完成后规划的后续阶段（已全部完成，保留作历史排期记录）：

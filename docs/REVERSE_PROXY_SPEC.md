...
- 管理端 Codex create/import/update 的 refresh-token-only 预刷新允许在账号尚未持久化、没有 `account_id` 时调用 Reverse Proxy Runtime；运行期自动刷新仍对已持久化账号使用 per-account single-writer guard。

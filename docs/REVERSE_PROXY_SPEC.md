...
当前 Reverse Proxy Runtime 支持一个账号 metadata 驱动的 Egress Profile 子集：
`metadata.egress_profile` 或兼容的扁平 metadata key 可声明 `user_agent`、
`extra_static_headers` / `header_set_template`、`forbidden_headers`、
`accept_language`、`sec_ch_ua_template`、`http_version_policy=require_h1|prefer_h1`
...
- WP-470 将 OpenAI-compatible Realtime `GET /v1/realtime` 的非 API-key 账号绑定到 Reverse Proxy Runtime：Gateway 解析 query `model`，调度具备 `realtime_websocket` 能力的账号，Provider Adapter 构造上游 `/realtime?model=<mapped_upstream_model>` WebSocket session，Runtime 使用选中账号 OAuth/session/client-token credential 注入上游身份并双向 relay text/binary frames。WP-630 起，`runtime_class = api_key` 的官方 Realtime 账号由 Gateway 直连上游并只注入选中账号 API key；该路径不属于 2api Reverse Proxy Runtime。两条路径都不得把 caller 的 `Authorization`、`Cookie` 或 SRapi headers 透传给上游，也不是 `POST /v1/realtime`。
- WP-570 将 active realtime slot 安全摘要暴露到 `GET /api/v1/admin/ops/realtime/slots`，用于运维诊断；WP-590 起 Redis 可用时该摘要和 slot 限额跨 API 节点生效，本地降级模式只覆盖当前节点内存。该 AdminOps 接口只读取 slot lifecycle metadata 和 hash 后的 affinity key，不读取 Reverse Proxy Runtime credential/cookie jar，也不返回上游 realtime frames。
...
- WP-600 起，`codex_cli` OAuth refresh 使用 `https://auth.openai.com/oauth/token`、Codex CLI client ID `app_EMoamEEZ73f0CkXaXp7hrann`、`grant_type=refresh_token` 和 `scope=openid profile email`，除非账号 credential/metadata 显式覆盖 `oauth_token_url`、`oauth_client_id` 或 `oauth_scope`。
- WP-610 起，`claude_code_cli` OAuth refresh 使用 `https://api.anthropic.com/v1/oauth/token`、Claude Code client ID `9d1c250a-e61b-44d9-88ed-5944d1962f5e`、`grant_type=refresh_token` 和 JSON body，除非账号 credential/metadata 显式覆盖 `oauth_token_url`、`oauth_client_id` 或 `oauth_request_encoding`。
- WP-620 起，`antigravity_desktop` OAuth refresh 使用 `https://oauth2.googleapis.com/token`、Antigravity client ID `1071006060591-tmhssin2h21lcre235vtolojh4g403ep.apps.googleusercontent.com`、`grant_type=refresh_token` 和 form body。Antigravity refresh 需要账号加密 credential 显式提供 `oauth_client_secret` 或 `client_secret`；SRapi 不硬编码该 secret。
- 管理端 Codex / Claude Code / Antigravity create/import/update 的 refresh-token-only 预刷新允许在账号尚未持久化、没有 `account_id` 时调用 Reverse Proxy Runtime；运行期自动刷新仍对已持久化账号使用 per-account single-writer guard。
...
```txt
utls 完整 TLS impersonation（metadata opt-in HTTP/1.1 ClientHello 模板子集已支持，完整 JA3/JA4/代理/HTTP2 指纹仍暂缓）

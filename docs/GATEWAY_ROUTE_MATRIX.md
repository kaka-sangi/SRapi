...
| `/v1/images/edits` | Media runtime | 最小解析 OpenAI-compatible multipart image edit body，支持 `image` / `image[]`、可选 `mask`、`prompt/model` 与输出选项，调度后调用 OpenAI-compatible `/images/edits`，记录 media/usage 和调度证据。 | WP-480 |

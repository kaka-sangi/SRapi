...
| `/v1/responses/compact` | Responses compact runtime | OpenAI Responses compact 子资源；复用 Gateway auth/model policy/Scheduler/Provider Adapter/usage evidence 链路，`reverse-proxy-codex-cli` 同协议请求发送到 Codex `/responses/compact` 并原样回放 `response.compaction` JSON。跨协议不做伪转换。 | active |

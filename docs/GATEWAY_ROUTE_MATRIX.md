...
| `/v1/images/edits` | Media runtime | 最小解析 OpenAI-compatible multipart image edit body 和 JSON 本地图像引用，支持 `image` / `image[]` / `images`、可选 `mask`、`prompt/model` 与输出选项，调度后调用 OpenAI-compatible multipart `/images/edits`，记录 media/usage 和调度证据。Remote URL 和 `file_id` references 仍需后续安全边界。 | WP-480, WP-510 |

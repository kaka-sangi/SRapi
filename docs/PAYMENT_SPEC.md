...
状态：支付系统已实现并上线。EasyPay、Stripe、支付宝官方、微信支付官方四个渠道均已接入下单、Webhook 验签和履约；退款、优惠码、幂等、负载均衡和管理后台均已落地。本文档继续作为支付域的规范与边界约束（数据模型、订单状态机、安全要求），现有实现以本文档为准。
...
| 渠道 | 状态 | 说明 |
| --- | --- | --- |
| EasyPay | 已上线 | 兼容易支付协议，可承接支付宝/微信聚合支付。 |
| Alipay Official | 已上线 | 支付宝官方开放平台，已接入 PC Page Pay / H5 Wap Pay 下单和异步通知验签。 |
| WeChat Pay Official | 已上线 | 微信支付 APIv3，已接入 Native / H5 / JSAPI 下单和 APIv3 通知验签解密。 |
| Stripe | 已上线 | 国际银行卡和多币种支付。 |
| LDCPay | Roadmap / 尚未实现 | Linux DO Credit 或类似积分支付。 |
| Custom Webhook | Roadmap / 尚未实现 | 外部支付系统通过受控 API 入账（见 §11 外部集成说明）。 |

渠道实现位于 `apps/api/internal/modules/payments/`：`providers/checkout` 定义统一下单接口，`providers/stripe` 使用 `stripe-go/v78` 创建 Stripe Checkout Session 并由 Stripe webhook SDK 验签，`providers/easypay` 生成带签名的 EasyPay 跳转 URL，`providers/alipay` 使用 `smartwalle/alipay/v3` 生成支付宝 Page/Wap Pay 支付 URL，并由 `service` 使用支付宝公钥验签异步通知。`providers/wechat` 使用 `wechatpay-apiv3/wechatpay-go` 创建微信 Native / H5 / JSAPI 预支付订单，并由 `service` 使用微信 APIv3 通知签名验证和 AES-GCM 解密后复用现有幂等、金额校验和履约链路。管理员侧已支持 provider instance 的创建、更新和本地配置测试；测试接口只解密并校验必需配置，不发起外部扣款或网络请求。Stripe test-mode 充值闭环已有 `make smoke-payment-stripe` 入口；Alipay Page Pay checkout smoke 已有 `make smoke-payment-alipay` 入口，并可选用本地签名通知验证 SRapi webhook 链路；WeChat Pay APIv3 prepay smoke 已有 `make smoke-payment-wechat` 入口，并可选用本地签名加密通知验证 SRapi webhook 链路；真实沙箱/商户外部回调仍需对应渠道凭证和平台通知演练。
...
所有渠道回调统一走单一带渠道占位符的路由（`provider` 取 `stripe` / `easypay` / `alipay` / `wechat`）：

```txt
POST /api/v1/webhooks/payments/{provider}
```

回调归属按 payload 中的本地订单号对应订单绑定的 provider instance 验签，避免多商户实例互相抢签。该路由无需用户态鉴权（`security: []`），仅依赖渠道签名校验。
...
以 `packages/openapi/openapi.yaml` 为权威定义；下列为当前已上线的支付路由。
...
Webhook：见 §8 的 `POST /api/v1/webhooks/payments/{provider}`。

外部集成（Roadmap / 尚未实现）：原计划提供独立的 `payment-integrations` 受控入账/确认接口（外部发卡、积分或对账系统通过独立权限和审计日志入账，不复用普通用户 API Key）。该接口目前尚未在 OpenAPI / 代码中实现；如需外部入账，可走管理员侧的退款/订单接口或后续按本节约束补齐。
...
| 阶段 | 内容 | 状态 |
| --- | --- | --- |
| 数据模型与接口 | 数据模型、订单状态机、OpenAPI、模块边界。 | 已交付 |
| EasyPay + 订单链路 | EasyPay、支付订单、Webhook、余额充值、管理后台。 | 已交付 |
| 官方渠道 + 退款 | 支付宝/微信官方、Stripe、退款、邀请返利联动。 | 已交付 |
| 外部集成 / LDCPay | 外部支付集成（`payment-integrations`）、LDCPay 积分支付、自定义渠道受控入账。 | Roadmap / 尚未实现 |

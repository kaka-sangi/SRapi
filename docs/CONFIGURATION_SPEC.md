...
- API Key auth cache。
- Gateway API key/user RPM、API key TPM、provider account RPM/TPM rate limit counters，以及 API key concurrency ZSet leases；Redis 不可用时 release mode 必须启动失败，非 release mode 会禁用 Gateway rate limit。
- account lease。

...
> 策略名取自实现 `apps/api/internal/modules/scheduler/contract/contract.go`
> 中的 `StrategyName` 常量（另含 `quota_protect`、`sticky_first`）。早期别名
> `quality_first` / `cache_affinity` / `low_latency` 仍被网关接受并归一化为上述
> 规范名（见 `normalizeSchedulerStrategyName`），新场景请直接使用规范名。
...
缓存亲和评分（场景 H/I）已实现：`cache_affinity_first` 策略与 `cacheScore`
打分逻辑均已落地，并由 `TestCacheAffinityFirstPrefersHealthyCachedCandidate`、
`TestCacheAffinityDoesNotOverridePoorHealth` 等单元测试覆盖
（见 `apps/api/internal/modules/scheduler/service`）。
...
strategies: balanced, cost_saver, premium_quality, cache_affinity_first, latency_first
```

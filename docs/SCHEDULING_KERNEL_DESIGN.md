# SRapi Scheduling Kernel 调度内核设计依据与现状

> Status: 调度内核已实现并在生产路径运行。本文档是「设计依据 + 现状」参考——记录调度内核的目标、模型、打分公式与权衡（why / rationale）。实现级契约（过滤、打分、Lease、Decision、Feedback、测试）以 `SCHEDULER_V1_SPEC.md` 为准，策略扩展见 `SCHEDULER_STRATEGY_EXTENSION_SPEC.md`。各组件与表的落地状态见 §15 / §18 / §19 的 Status 标注。
...
> Status: 部分已落地。`scheduler_decisions`、`scheduler_feedbacks`、`scheduler_strategies`、`account_health_snapshots`、`account_quota_snapshots` 已作为物理表实现（见 apps/api/migrations/postgres/up/000001_initial_schema.sql 及 ent schema）。`sticky_sessions` 与 `cache_affinity_records` 是设计参考字段表：会话粘度经 Redis 会话亲和绑定实现，cache affinity 命中记录在 `scheduler_decisions.cache_affinity_hit` 列与运行期评分中，二者目前未作为独立物理表落库。
...
> Status: 已实现。前端已提供调度决策、策略对比与调度回归测试三个面板。
...
```txt
/scheduler-decisions        调度决策与评分明细（含 sticky / cache affinity 命中、reject reasons）
/admin/ops/strategy         策略效果对比（Strategy Comparison）
/admin/scheduled-tests      调度回归测试计划与模拟器（scheduled-test-plans）
```

注：早期草案中的 `/admin/scheduler/*` 路径未采用，以上为已落地的真实路由。

## 19. 实现范围

> Status: 已实现。本节列出的内核组件全部落地（apps/api/internal/modules/scheduler/service、ent schema `schedulerdecision` / `schedulerfeedback`、迁移 000001 / 000004 / 000011）。
...
- CandidateBuilder
- PolicyFilter
- ScoreEngine（含 Cost / Latency / Quality 三目标 Pareto 前沿，见 4.5）
- LeaseManager
- FeedbackCollector
- 策略模板：balanced / cost_saver / latency_first / quota_protect / sticky_first / cache_affinity_first / premium_quality（StrategyName，见 contract）
- Soft / hard / cache-only sticky session（会话粘度，经 Redis 会话亲和绑定）
- health score 与 quota score
- scheduler_decisions / scheduler_feedbacks 记录回流

实现级约束以 `SCHEDULER_V1_SPEC.md` 为准。本文档承担「设计依据 + 现状」角色，记录调度内核的目标、模型与权衡；`SCHEDULER_V1_SPEC.md` 描述实现必须通过的过滤、打分、Lease、Decision、Feedback 和测试规则。

Roadmap / 尚未实现（仍为规划项，代码中暂无对应实现）：

...
- xAI/Grok `native-grok` Responses WebSocket relay 使用同一 `/v1/responses/ws` Gateway runtime：显式带 `upstream_ws` / `responses_websocket` / `xai_responses_websocket` 时，Scheduler 要求 `responses_websocket.v1`，Provider Adapter 构造 xAI/Grok `ws/wss` `/responses` session，首帧为规范化 `response.create`，强制 mapped upstream model，移除 `stream` / `background`，并用 `prompt_cache_key` 或显式 `x_grok_conv_id` 作为 `x-grok-conv-id`。API-key 账号由 Gateway 直连并注入选中账号 API key；非 API-key runtime 仍通过 Reverse Proxy Runtime 注入选中账号身份。

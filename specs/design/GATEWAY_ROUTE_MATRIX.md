...
OpenAI-compatible preset:
  ...
  GET  {alias}/v1/responses/ws        (only when preset advertises responses_websocket)
  GET  {alias}/v1/realtime            (only when preset advertises realtime_websocket)
...
Provider alias 只通过 `/api/provider/{provider_key}` 路由族暴露。所有 alias 只强制对应 `provider_key`，并把原始 alias path 写入 usage log、realtime slot 与 scheduler decision 的 `source_endpoint` 证据；协议归一化和 Provider Adapter 仍使用标准 Gateway endpoint。WebSocket alias 不是独立长连接 runtime；capability-gated `{alias}/v1/responses/ws` 与 `{alias}/v1/realtime` 进入标准 WS handler，只增加 forced provider context。
...
Grok/xAI 使用独立 `xai_compatible` preset family 和 `native-grok` adapter type。REST wire shape 仍是 OpenAI-compatible，但能力基线由 Grok preset 定义，不继承通用 OpenAI-compatible 子资源；API-key upstream 与后续 Grok Web session 都归属 `grok` provider，但 runtime_class 不同。
...
| `/v1/images/generations` | Media runtime | 最小解析 OpenAI image generation body，调度前要求 `image_generations` effective capability，调度后调用 OpenAI-compatible `/images/generations`，记录 media/usage 和调度证据。`stream=true` 在 OpenAI-compatible API-key / 普通 reverse-proxy 和 Codex image paths 都返回下游 `image_generation.partial_image` / `image_generation.completed` SSE，并从最终事件解析 usage。 | Shipped (WP-290) |
| `/v1/images/edits` | Media runtime | 最小解析 OpenAI-compatible multipart image edit body 和 JSON 本地图像引用，支持 `image` / `image[]` / `images`、可选 `mask`、`prompt/model` 与输出选项；JSON `image`/`images`/`mask` 只接受本地 data URL / base64 payload。调度前要求 `image_edits` effective capability；OpenAI-compatible API-key / 普通 reverse-proxy accounts 调用 multipart `/images/edits`，`reverse-proxy-codex-cli` accounts 转换为 Codex `/responses` `image_generation` tool `action=edit`，记录 media/usage 和调度证据。`stream=true` 会把 Codex、Responses-style OpenAI image events 和原生 OpenAI-compatible image events 统一转换为下游 `image_generation.partial_image` / `image_generation.completed` SSE。Remote URL 和 `file_id` references 仍需后续安全边界。 | Shipped (WP-480, WP-510, WP-520) |
...
| `/v1/responses/ws` | Realtime/WS runtime | WebSocket transport for Responses-compatible `response.create` events; by default each frame still enters the standard Responses Gateway runtime, Scheduler, Provider Adapter, usage log, and scheduler decision chain. Query/header sticky hints feed Scheduler session affinity. Provider aliases such as `/api/provider/xai/v1/responses/ws` enter this same runtime, force `provider_key`, and preserve the alias path as `source_endpoint`. With explicit `upstream_ws` / `responses_websocket` opt-in, SRapi requires `responses_websocket` effective capability and relays to the selected provider's Responses WebSocket session. Eligible `reverse-proxy-codex-cli` accounts use selected account OAuth/session/CLI credentials through Reverse Proxy Runtime; eligible `native-grok` API-key accounts connect directly to xAI/Grok `ws/wss /responses` with selected account API key, while non-API-key Grok runtime identities use Reverse Proxy Runtime. Realtime slot lifecycle and deploy-level slot limits run before WebSocket upgrade; active slot summaries are exposed at `GET /api/v1/admin/ops/realtime/slots`; those limits and summaries are Redis-backed across API nodes when Redis is available, with local in-memory fallback only outside release mode. Additional Claude Code / Antigravity provider-native realtime adapters remain Roadmap. | Shipped (WP-380, WP-410, WP-460, WP-570, WP-590) |

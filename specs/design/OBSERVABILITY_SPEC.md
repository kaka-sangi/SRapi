...
AdminOps error-log feed 使用 `ops_error_logs` 作为持久化证据源：
...
该 feed 保存 request/trace/user/API key/account/provider/model/status/error
class/error phase/error owner/source、source/target protocol、attempt number、
latency、估算 token、上游 request id、脱敏 body excerpt、脱敏 attempt history 和
resolution metadata。它用于排障和人工处理状态，不保存原始请求体、headers、
prompt、credential 或 replay payload，不承担 retry/replay workbench 的职责。若需要
重放分析，必须使用已脱敏的 scheduler request snapshots 或 idempotency evidence，
而不是把完整请求体塞入错误日志。

`GET /api/v1/admin/ops/error-logs/fingerprints` 提供基于 `ops_error_logs`
的实时错误指纹摘要，用于告警后的快速归因。它按 source endpoint、target
protocol、model、status class/code、error class/phase/owner/source 和归一化
message pattern 聚合，并返回 count、resolution 分布、最近样例日志和 live scan
meta（window、scanned、truncated）。指纹 key 不得包含 request id、API key、user /
account / provider 原始标识、body excerpt、prompt、credential 或 cookie。该接口是短
窗口只读摘要，不替代后续独立 rollup 表。
...
控制面已内置 email 通知通道和投递证据（`obs_notification_channels` / `obs_notification_deliveries`）。规则触发或恢复时，operations service 会按通道状态、最低 severity 和 `send_resolved` 生成幂等 delivery；`alert_notifications` worker 使用部署环境中的 SMTP sender 异步投递，并把成功/失败、attempt count、last error 和下一次重试时间写回 delivery 表。投递记录不复制 alert `details_json`，只返回 channel、target、alert summary 和时间戳等低敏证据。

Roadmap / 尚未实现：除 email 外的外部通道和凭证管理。
...
> Prometheus + Alertmanager 部署侧路由仍可作为外部告警出口（见 §3.3 末尾的 `deploy/prometheus-srapi-alerts.yaml` / `deploy/alertmanager.yml`）。SRapi 内置 email 通道复用 `EMAIL_SMTP_*` 部署环境变量；后续若增加 webhook/IM 通道，通知凭证必须保存到 secret settings，不得进入前端响应。
...
obs_notification_channels    # ent: obsnotificationchannel（当前仅 email）
obs_notification_deliveries  # ent: obsnotificationdelivery
...
obs_error_fingerprints       # 独立错误指纹聚合表（当前为 ops_error_logs 实时摘要 + obs_alert_events 内联 fingerprint）
```

> `notifications` 模块面向事务性认证邮件与用户订阅偏好，不是 Ops 告警通道。Ops email channel 只复用该模块的 SMTP sender adapter，不复用用户通知偏好或 outbox 事件目录。
...
- `obs_notification_channels` / `obs_notification_deliveries` 持久化内置 email 告警通道和投递证据，CRUD / list 见 §11 的 `/notification-channels{,/{id}}` 与 `/notification-deliveries`。
...
- 内置 email 告警通知通道与投递证据（`obs_notification_channels` / `obs_notification_deliveries`，§7、§10、§11）。
- `ops_error_logs` 实时错误指纹摘要（`GET /api/v1/admin/ops/error-logs/fingerprints`，§4）。
...
- 独立错误指纹聚合表（`obs_error_fingerprints`；当前为 `ops_error_logs` 实时摘要 + 告警事件内联 fingerprint）。
- 非 email 外部告警通知通道（webhook/钉钉/飞书/企业微信）和 secret-backed 凭证管理；部署侧 Prometheus + Alertmanager 路由仍可独立使用（§3.3 末尾）。

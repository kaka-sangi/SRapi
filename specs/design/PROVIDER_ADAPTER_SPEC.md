...
- `/v1/images/generations`, `/v1/images/edits`, and `/v1/images/variations` on `reverse-proxy-chatgpt-web` must use ChatGPT Web official-client image flow, not OpenAI-compatible `/images/*`: upload edit/variation source images through `/backend-api/files` when present, prepare via `/backend-api/f/conversation/prepare`, run `/backend-api/f/conversation` with `picture_v2`, extract generated `file-service://` / `sediment://` image references, resolve ChatGPT Web download metadata, and return OpenAI image response data. Image edit `mask` is represented by composing the mask alpha into the uploaded source image before the official conversation request. This is a gateway synchronous bridge; long-running task storage belongs in a separate media job module, not inside the adapter.
...
WP-290 起，`supports_image_generations` 映射到 canonical `image_generations` endpoint capability。Gateway image generation 请求必须带 `image_generations` request capability；OpenAI-compatible API-key 和普通 reverse-proxy accounts 使用 `/images/generations` 上游路径，并解析 `url` / `b64_json` image outputs；`reverse-proxy-chatgpt-web` 使用 ChatGPT Web `/backend-api/f/conversation` official-client 图片流并解析 generated file references。`stream=true` 会向上游发送 image stream 请求，并把 Responses-style `response.image_generation_call.partial_image` / `response.completed` 或原生 `image_generation.partial_image` / `image_generation.completed` 统一转换为下游 `image_generation.partial_image` / `image_generation.completed` SSE。WP-480 起，Gateway image edit 请求使用独立 `image_edits` request capability；OpenAI-compatible API-key 和普通 reverse-proxy accounts 使用 multipart `/images/edits` 上游路径，转发 `image` / `image[]`、可选 `mask` 和输出选项，并解析同一 OpenAI-compatible image response。`reverse-proxy-chatgpt-web` image edit 不走 multipart 伪兼容路径：源图像先上传到 ChatGPT Web `/backend-api/files`，`mask` 在 adapter 内合成为 PNG alpha，随后以 `image_asset_pointer` + prompt 进入 `/backend-api/f/conversation` official image flow。`reverse-proxy-codex-cli` 账号不走 OpenAI multipart 伪兼容路径，而是把 canonical image edit 转换为 Codex `/responses` 请求：源图像作为 user message `input_image` data URL，`mask` 作为 `tools[].input_image_mask.image_url`，tool 固定为 `type=image_generation`、`action=edit`，并转发 `size`、`quality`、`background`、`output_format`、`output_compression`、`moderation`、`input_fidelity` 和 `partial_images`。WP-510 起，下游 JSON image references 会在 HTTP/Gateway 层解码成同一个 canonical image edit request；remote URL 和 `file_id` references 仍在 Gateway 层拒绝。WP-520 起，`stream=true` 的 image edit 请求仍经过同一 Provider Adapter 和 usage 证据链；Codex、OpenAI-compatible API-key 和普通 reverse-proxy image edit streams 都会转换为下游 `image_generation.partial_image` / `image_generation.completed` 事件，ChatGPT Web image edit 当前为同步 official flow。WP-490 起，Gateway image variation 请求使用独立 `image_variations` request capability；OpenAI-compatible API-key 和普通 reverse-proxy accounts 使用 multipart `/images/variations` 上游路径，转发单个 source `image`、`n`、`size`、`response_format` 和 `user`，并解析同一 OpenAI-compatible image response；`reverse-proxy-chatgpt-web` variation 上传 source image 后以 reference + variation prompt 进入 ChatGPT Web official image flow。OpenAI 官方 upstream 当前仅支持 `dall-e-2`，SRapi 通过 model mapping 决定上游模型名。SRapi 不使用 `images` 伞形 endpoint capability；生成、编辑和变体必须分别声明。
...
- embeddings、images（generations / edits / variations，含 OpenAI-compatible/Codex progressive image SSE 转换）、moderations、rerank、audio transcriptions / speech 等扩展端点（§6、§22）。
...
- `reverse-proxy-chatgpt-web` 文本请求必须构造 ChatGPT Web Conversation / official-client 形态，POST 到 ChatGPT Web origin 下的 `/backend-api/conversation`；图片生成、编辑和变体请求必须构造 ChatGPT Web `/backend-api/f/conversation/prepare` + `/backend-api/f/conversation` 图片形态并解析 generated file references；以上都通过 Reverse Proxy Runtime 注入选中账号 OAuth/session token 身份；不得退化为 OpenAI-compatible `/chat/completions` 或 `/images/generations` / `/images/edits` / `/images/variations`，也不得接受 `runtime_class = api_key` 作为 2api 身份。
- `reverse-proxy-codex-cli` 文本请求必须构造 Codex Responses / official-client 形态，POST 到配置的 Codex base URL 下的 `/responses`；`/v1/responses/compact` 请求必须要求 `responses_compact` effective capability，POST 到 `/responses/compact` 并保留 `response.compaction` 原始 JSON。`GET /v1/responses/{response_id}/input_items` 必须要求 `responses_input_items` effective capability，通过选中账号调用上游 `/responses/{response_id}/input_items` 并原样回放 JSON list，SRapi query `model` 只用于调度不得转发上游。以上请求都通过 Reverse Proxy Runtime 注入选中账号 OAuth/session/CLI token 身份；不得退化为 OpenAI-compatible `/chat/completions`，也不得接受 `runtime_class = api_key` 作为 2api 身份。
- Codex official-client headers 默认使用 `Originator: codex_cli_rs`；账号 metadata 可以用 `codex_originator`（兼容 `originator`）显式覆盖，以适配 Codex Desktop、VS Code、Atlas、SDK 等官方客户端家族形态。默认 instructions 只在普通 `/responses` 缺省时注入，账号 metadata 可用 `codex_default_instructions`（兼容 `default_instructions`）覆盖；`/responses/compact` 不注入默认 instructions / base prompt，但缺省或空白 `instructions` 必须规范化为显式空字符串，避免 Codex upstream 拒绝缺失字段。

...
- `reverse-proxy-chatgpt-web` 文本请求必须构造 ChatGPT Web Conversation / official-client 形态，POST 到 ChatGPT Web origin 下的 `/backend-api/conversation`；图片生成请求必须构造 ChatGPT Web `/backend-api/f/conversation/prepare` + `/backend-api/f/conversation` 图片形态并解析 generated file references；以上都通过 Reverse Proxy Runtime 注入选中账号 OAuth/session token 身份；不得退化为 OpenAI-compatible `/chat/completions` 或 `/images/generations`，也不得接受 `runtime_class = api_key` 作为 2api 身份。

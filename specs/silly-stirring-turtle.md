...
|---|---|---|---|
| **A1 Session 持久化** | `runtime_state.go:136` 硬编码 `authmemory.New()`；session 用 32B 随机 + `sess` 前缀 + CSRF token，TTL 12h（`modules/auth/service/service.go:16`） | Password 登录、CSRF 防护、credential_version 框架完整 | **新增 `entstore/auth/store.go`**（参考 `entstore/accounts/store.go:86-94` 模式）+ 新增 `AuthSession` ent schema |
| **A2 限流中间件** | `apikey.go:29-30` 有 `RPMLimit/TPMLimit`，`user.go:26` 有 `rpm_limit`；`server.go:209-352` 中间件链**没有限流**；`go.mod:11` 已有 `redis/go-redis/v9 v9.19.0` | 已有 30s cooldown 字段（`server.go:71` `rateLimitCooldownWindow`）；Account 层 `rpm_used/rpm_limit` metadata 已存（`runtime_gateway_core.go:650-666`） | **新增 `platform/ratelimit/`** Redis 令牌桶/滑动窗口；在 `scheduleGatewayRequest` 前插入 `validateRPMTPMQuota`；scheduler `rejectReason()` 加 `rpm/tpm_limit_exceeded`（`scheduler/service/service.go:190`） |
| **A3 账号健康 Worker** | `AccountHealthSnapshot` schema 完整（success_rate / latency_p50/p95 / circuit_state / cooldown_until）；`runtime_gateway_core.go:652-659` scheduler 已读它，但**没有 worker 主动写** | `accounts.LatestHealthSnapshotByAccount()` 已实现；`circuit_state` 字段已留 | **新增 `accounts.ProbeAccount()`** 探测上游；**新增后台 worker**（参考 `workers/outbox/`、`workers/retention/`）每 5min 遍历活跃 account；状态机：3 连续错误 OR error_rate > 50% → unhealthy |
| **A4 跨厂商故障转移** | `gatewayCandidates()` 在 `runtime_gateway_core.go:599-648` 已遍历所有 provider 的 account，**已经天然支持跨厂商候选**；但 `ScheduleResult` 只返回 1 个 Candidate；`Decision.AttemptNo` 字段已留 | Filter reason 已枚举到 9 类（`scheduler/service/service.go:89`）；sticky session、account group 隔离已实现 | **ScheduleResult 加 `[]Candidate` 排序列表**；HTTP handler 包裹 retry loop；Decision 加 `fallback_from_decision_id` |
| **A5 多供应商扩展** | Adapter 已实现 5 种：openai/anthropic/gemini 兼容 + codex/antigravity 反代（`provider_adapters/service/service.go:36-73`）；Provider 数据库驱动；`bootstrapGatewayCatalog()` 仅 seed 一个 openai-compatible | `capabilities map` + `adapter_type` + `protocol` 三维度可组合；credential 加密+版本框架完整 | **新增 `generic-reverse-proxy` adapter**（接 metadata `{base_url, auth_header, body_map}`）；**新增 `providers/preset/`**（DeepSeek/Kimi/通义/智谱/Grok/Mistral/Groq/Together 配置）；完善 `/admin/providers/{id}/test` |
...
| **C3 PII / Injection** | 仅 `/v1/moderations` 端点**转发到上游**（`provider_adapters/service/moderations.go`）；全局无 pii/guardrail/injection 模块；`safety_settings_ignored` 警告仅记录不处理 | Gateway pipeline 有 CanonicalRequest 阶段可挂 hook；Canonical IR 设计干净 | **新增 `modules/content_safety/`**（pii_detector regex 起步 / guardrail_engine / injection_detector）；在 CanonicalRequest 后插入 SafetyMiddleware；`anonymizeRequest()` 脱敏 + 审计 |

### Pareto 路由扩展点（杀手锏 K1）

**当前完成度高于预期**——`StrategyRegistry` 框架已成型，scoreBreakdown 已有 10 维度（Final/Health/Quota/Latency/Sticky/Cache/Cost/Fairness/RiskPenalty/SaturationPenalty），SchedulerStrategy 表已支持持久化策略配置；`docs/SCHEDULER_STRATEGY_EXTENSION_SPEC.md` 已设计 9 个策略和灰度机制。
...
| **Pareto 前沿优化** | 当前是简单加权求和 | 新增 `pareto_optimizer.go`：多目标 Pareto 支配关系筛选 |
| **策略灰度/dry-run** | spec 已设计 SimulateStrategy 未实现 | 新增 `strategy_simulator.go`：shadow decision 不执行；按 % 灰度新策略 |
...
- **数据库迁移**：单一 866 行 initial schema，**无增量迁移工作流**——这是个隐患，因为我们要加很多表
- **Atlas 配置**：未见 `atlas.hcl`，需要确认 Ent → Atlas → migration 的实际工作流
...
- 要点：用 `ent migrate diff` 生成 SQL；约定文件名 `0000NN_short_subject.up.sql` + `.down.sql`；CI 中跑 `migration-check`
- 验收：`make migration-diff` 能产出可执行 SQL；CI 全绿
- 依赖：无
...
- 文件：新增 `apps/api/ent/schema/authsession.go`、`apps/api/internal/persistence/entstore/auth/store.go`、迁移 `000002_auth_sessions.up.sql/.down.sql`；改 `runtime_state.go:100, 136` 默认存改为 entstore
...
- 文件：新增 `apps/api/internal/platform/ratelimit/{token_bucket.go, sliding_window.go, redis_lua.go}`；新增 `apps/api/internal/httpserver/middleware_ratelimit.go`；改 `server.go:352` 中间件链
- 要点：Lua 脚本原子化"判定 + 扣减 + 写回"；Key 模板 `srapi:rl:{kind}:{owner_id}:{window}`；超限返回 429 + `Retry-After`；并发用 Redis ZSet + 过期清理
- 验收：写一个测试程序快速调用直到被限流；p99 中间件耗时 ≤ 2ms（benchmark）
- 依赖：I2
...
- 要点：worker 用 sync.WaitGroup + ctx.Done() 优雅退出；并发受 `MaxConcurrentProbes` env 限制；3 连续错误 OR error_rate > 50% → status 转 `cooling_down`，并写 `cooldown_until`
...
- 文件：`apps/api/internal/modules/scheduler/contract/contract.go` 的 `ScheduleResult` 加 `Candidates []ScoredCandidate`；ent schema `schedulerdecision.go` 加 `fallback_from_decision_id` 字段 + 迁移
...
- 文件：新增 `apps/api/internal/modules/content_safety/{contract, service, store/memory}`；在 `gateway/service/service.go` Normalize 之后插入 SafetyMiddleware
- 要点：PII 起步用 regex（邮箱、手机、身份证）+ 配置化阈值；injection 起步用关键词 blacklist（"ignore previous instructions" 等 LLM-Guard 通用规则）；hit 后行为可选 `block / mask / warn`，记入 AuditLog
- 验收：发一个包含手机号的 prompt，被自动 mask
- 依赖：I2
...
- 要点：输入 candidates + (cost, latency, quality) 三维；输出 Pareto 前沿子集（任一维度上没有被其他候选支配）；然后再用 strategy 权重在前沿内排序
- 验收：mock 3 个候选（A 最便宜慢、B 最快贵、C 中等），Pareto 前沿应包含 A 和 B、排除被支配的
- 依赖：K1.1, K1.4

**[K1.6] strategy_simulator（dry-run + shadow，2 PD）**
- 目标：能用历史流量"假装"用新策略跑一遍，对比效果
- 文件：新增 `apps/api/internal/modules/scheduler/service/simulator.go`；新 API `POST /api/v1/admin/scheduler/strategies/{id}/simulate`
- 要点：从 SchedulerDecision 表回放 N 条请求 → 用新策略重算评分但不真调用上游 → 输出 hypothetical_selected vs actual_selected diff 报告
- 验收：把 balanced 当成"新策略"模拟跑过去 1000 条决策，diff 报告显示 100% 一致
- 依赖：K1.2

**[K1.7] /admin/ops/strategy 前端策略对比仪表盘（3 PD）**
- 目标：管理员可视化对比两个策略在同一时间窗口的成本/延迟/质量
- 文件：`apps/web/src/app/admin/ops/strategy/page.tsx` + 子组件；后端 `runtime_admin_dashboard_ops_handlers.go` 加 `handleAdminOpsStrategyComparison`
- 要点：复用现有 dashboard 数据源；图表用 recharts；过滤维度（time range / model / provider）
- 验收：浏览器打开页面，能看到两条策略的并排曲线
- 依赖：K1.6
...
000003 scheduler_decision_fallback_field    （A4.1）
000004 workspaces_and_user_workspace_id     （C3.1）
000005 role_permissions_and_entitlements    （C3.2）
000006 quality_evaluations                  （K1.4）
000007 usage_logs_charged_at_index          （B1.2 性能索引）
```

每个迁移配套 `down` 脚本；每次合并前 `make migration-check` 必须通过。

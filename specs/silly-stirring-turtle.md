...
- 新增 smoke：
  ...
  3. `smoke-failover`：已落地为 `make smoke-failover`，创建两个临时 OpenAI-compatible Provider 和本地 mock upstream，验证 primary 503 后自动切到 secondary，并核验 usage attempts、`fallback_from_decision_id`、`fallback_excluded` 和 `srapi_gateway_failover_total`
  4. `smoke-rate-limit`：已落地为 `make smoke-rate-limit`，使用 `rpm_limit=1` 临时 API Key 验证第二个 Gateway 请求返回 429、`rpm_limit_exceeded` 和 `Retry-After`
  5. `smoke-quality-eval`：已落地为 `make smoke-quality-eval`，使用本地 judge 验证 QualityEval sample capture、worker evaluation 和 Scheduler quality evidence；真实外部 judge 比例 smoke 仍需生产凭证
  6. `rate-limit-bench`：已落地为 `make rate-limit-bench`，对真实 Redis 执行 Allow / AcquireConcurrency / ReleaseConcurrency p99 ≤ 2ms guard

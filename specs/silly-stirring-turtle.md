...
| **C2 多租户三层** | 已新增 `Workspace` Ent/PostgreSQL 表；`users.workspace_id` 与 `api_keys.workspace_id` 均为 nullable；User 持久化创建默认生成 personal workspace，API Key 未指定 workspace 时继承 owner user；`AccountGroup` 仍是 provider 分组**不是多租户**；`Role` 表仍仅 name+description；无 Org/Team/Entitlement 表 | UserRole 多对多已留；APIKey 已有 scopes_json/allowed_models_json；`000008_workspaces_and_user_workspace_id` 迁移含 existing user/API key 幂等回填 | 后续 C3.2：Role 加 `permissions` JSON；新增 `Entitlement` 表；再后续才引入 Organization/Team |

...
- 状态：已落地（API key/user RPM + API key TPM 已通过 Redis Lua 原子 enforcement；API key `concurrency_limit` 已通过 Redis ZSet 租约 enforcement；`make rate-limit-bench` 对真实 Redis 执行 Allow / AcquireConcurrency / ReleaseConcurrency p99 ≤ 2ms guard）。

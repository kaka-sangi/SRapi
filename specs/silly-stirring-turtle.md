...
| 子任务 | 现状关键证据 | 我们预留的资产 | 还缺什么 |
|---|---|---|---|
| **C1 OTel + 结构化 trace** | 已落地 `go.opentelemetry.io/otel` tracing SDK + OTLP gRPC exporter 配置、app 生命周期初始化/Shutdown、HTTP server root span、W3C trace context 提取、slog context handler 自动注入 `request_id` / `trace_id` / `user_id` / `api_key_id`；`scheduler.Schedule`、`payments.HandleWebhook`、`accounts.ProbeAccount` 已补关键 service span、业务 outcome、低敏诊断属性和 `error.type` 错误分类；`/metrics` 已由 Prometheus client SDK 渲染，Gateway duration 与 provider probe latency 均暴露为 histogram bucket（0.05/0.1/0.25/0.5/1/2.5/5/10s + `+Inf`）；`slo_evaluator` worker 已每分钟用 `obs_slo_definitions` + `usage_logs` 评估 availability SLO 多窗口 burn-rate，并只自动创建/更新/恢复 `slo.burn_rate.*` 安全告警事件 | Prometheus 指标可扩展；context 传 request_id/user_id/api_key_id/trace_id；OBS 表设计完整；`platform/otel.StartSpan` / `EndSpan` 可复用到后续关键 service；SLO 告警 details 只保存窗口、计数和 burn-rate 数值 | 后续补真实 Jaeger/Tempo collector smoke；保持现有指标名和低基数 label 语义 |

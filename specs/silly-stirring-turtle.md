...
- 状态：部分落地（单请求 dry-run + shadow strategy、稳定 rollout 预览、真实调度的脱敏 request/candidate snapshot 持久化已完成；批量历史回放 API 和真实流量灰度百分比仍待后续 K1.6 扩展）。
- 目标：能用请求 profile 假装用新策略跑一遍，对比 current vs shadow 的选择效果
- 文件：新增 `apps/api/internal/modules/scheduler/service/simulator.go`；新 API `POST /api/v1/admin/scheduler/simulate`；新增 `scheduler_request_snapshots` 作为后续历史 replay 证据源

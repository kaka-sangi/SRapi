...
| **A4 跨厂商故障转移** | `gatewayCandidates()` 已遍历所有 provider 的 account，天然支持跨厂商候选；`ScheduleResult.Candidates` 已返回排序候选列表；`SchedulerDecision.fallback_from_decision_id` 已落库并暴露到管理 API；lease update 已按 `(request_id, attempt_no)` 定位，避免 fallback attempt 互相覆盖；Gateway text / Responses / Messages / Embeddings / Gemini GenerateContent handler 已按 retryable provider error 自动切换候选；`usage_logs` 已按 attempt 记录，`srapi_gateway_failover_total` 已暴露；`make smoke-failover` 已可用本地 mock primary 503 → secondary 成功并核验 usage/decision/metric evidence | Filter reason 已枚举到 9 类；sticky session、account group 隔离、attempt_no、fallback_excluded reject evidence、failover usage evidence、failover metric 已实现 | 后续可补 streaming provider 直通场景的首字节前 retry 边界测试；A4 核心调度 + HTTP fallback + 本地 failover smoke 已完成 |
...
| 子任务 | 现状关键证据 | 我们预留的资产 | 还缺什么 |
|---|---|---|---|
| **C1 OTel + 结构化 trace** | 已落地 `go.opentelemetry.io/otel` tracing SDK + OTLP gRPC exporter 配置、app 生命周期初始化/Shutdown、HTTP server root span、W3C trace context 提取、slog context handler 自动注入 `request_id` / `trace_id` / `user_id` / `api_key_id`；`scheduler.Schedule`、`payments.HandleWebhook`、`accounts.ProbeAccount` 已补关键 service span、业务 outcome、低敏诊断属性和 `error.type` 错误分类；`/metrics` 已把 Gateway duration 暴露为 histogram bucket（0.1/0.5/1/5s + `+Inf`）；`slo_evaluator` worker 已每分钟用 `obs_slo_definitions` + `usage_logs` 评估 availability SLO 多窗口 burn-rate，并只自动创建/更新/恢复 `slo.burn_rate.*` 安全告警事件 | Prometheus 指标可扩展；context 传 request_id/user_id/api_key_id/trace_id；OBS 表设计完整；`platform/otel.StartSpan` / `EndSpan` 可复用到后续关键 service；SLO 告警 details 只保存窗口、计数和 burn-rate 数值 | 后续补真实 Jaeger/Tempo collector smoke；若替换为 Prometheus client SDK，需要保持现有指标名和低基数 label 语义 |
...
| 子任务 | 现状关键证据 | 我们预留的资产 | 还缺什么 |
|---|---|---|---|
| **B1 计费引擎** | 已落地 `UsageLog.cost` 回写、`charged_at` 扣费标记、`balance_charger` worker、`BillingLedger.usage_charge` 扣减、余额不足自动 suspend+audit、`GET /api/v1/me/balance`、`POST /api/v1/admin/pricing-rules:bulk` JSON/CSV dry-run 导入，以及 `000010_usage_logs_charged_at_index` pending scan 性能索引 | `BillingLedger` 支持 usage_charge / payment_credit / refund / adjustment / affiliate_transfer；Gateway pricing evidence 已串联 `pricing_rule / mapping_override / default_zero`；usage aggregation 和 dashboard 读 `cost` | 后续可补真实 10k usage/min 压测、按 workspace/org 的用量扣费分摊；B1 核心闭环已完成 |
| **B2 支付落地** | `payments/service` 已完成 CreateOrder / HandleWebhook / RequestRefund / fulfill / outbox/audit/billing/subscription 联动；HMAC-SHA256 webhook 验签 + 幂等 (`paymentauditlog.idempotency_key`)；AES-GCM 加密 provider config；Webhook 路由 `POST /api/v1/webhooks/payments/{provider}` 已注册；`payments/providers/checkout` 已抽象渠道 checkout，Stripe 通过 `stripe-go/v78` 创建 Checkout Session，EasyPay 会生成带签名的支付跳转 URL，Alipay Official 通过 `smartwalle/alipay/v3` 生成 Page/Wap Pay URL 并验签异步通知，WeChat Pay Official 通过 `wechatpay-apiv3/wechatpay-go` 创建 Native / H5 / JSAPI 预支付订单并验签解密 APIv3 通知；管理员侧已支持 `POST/PATCH /api/v1/admin/payments/providers` 和 `POST /api/v1/admin/payments/providers/{id}/test` 本地配置检查 | 订单状态机（pending → paid → fulfilled，含退款分支 8 状态）；provider_snapshot_json 防漂移；多商户实例选择；Stripe webhook SDK 验签和 checkout.session.completed 状态映射；EasyPay 聚合支付 URL 签名和现有 webhook 履约链路；Alipay Official RSA2 SDK 签名、支付宝公钥验签、金额校验、幂等和订单绑定 provider instance 归属均有服务层测试；WeChat APIv3 通知验签、AES-GCM 解密、金额校验、幂等和配置缺失检查均有本地测试；provider instance 更新会保持类型不可变并在重命名/替换配置时重新加密 config | 还缺 Stripe/支付宝/微信真实沙箱 smoke 外部凭证验证 |
| **B3 Affiliate** | `affiliate/service` 已实现 BindInvite / AccrueRebate / CompensateRefund / TransferToBalance；entstore 与 memory store 均支持 invite/relationship/rule/ledger；PaymentOrderPaid / PaymentOrderRefunded 已经由 outbox worker 分发到 affiliate accrual / refund compensation；TransferToBalance 在 entstore 中同事务写 affiliate ledger、billing ledger 和 user balance | 自邀请校验、过期码、激活状态、返利规则、幂等 reference、audit/outbox evidence 已留 | 后续可补用户侧 `GET /me/affiliate/ledger` / transfer-to-balance API、返利结算/提现产品化和 admin 批量结算 |
...
- 状态：已落地（API key/user RPM + API key TPM + provider account RPM/TPM 已通过 Redis Lua 原子 enforcement；API key `concurrency_limit` 已通过 Redis ZSet 租约 enforcement；`make rate-limit-bench` 对真实 Redis 执行 Allow / AcquireConcurrency / ReleaseConcurrency p99 ≤ 2ms guard）。
...
- 状态：已落地（scheduler 已读取 account metadata 的 `rpm_used` / `tpm_used` / `current_concurrency` 与 `rpm_limit` / `tpm_limit` / `max_concurrency`；Gateway 成功用量后会按最近窗口写回 account runtime quota metadata；Gateway 调度成功、上游 dispatch 前会用 Redis account RPM/TPM counter 做二次原子拦截；HTTP 回归证明 stale metadata 时 chat/failover 与 image direct-dispatch 路径都会 429 且不触达上游）。
...
- 状态：已落地（`github.com/stripe/stripe-go/v78` 已在 `go.mod`；`payments/providers/stripe` 实现 checkout provider，支持 Checkout Session 创建；`service/stripe_webhook.go` 用 Stripe webhook SDK 验签并映射 `checkout.session.completed`；单测覆盖 Stripe checkout metadata 与 webhook 履约）。
...
- 验收：单测已证明 checkout session metadata 与 webhook → balance ledger；生产 smoke 仍需真实 Stripe test mode 凭证完整跑一次充值 → webhook → balance 增加
- 依赖：B2.1 + B2.2
...
- 状态：已落地（`github.com/smartwalle/alipay/v3` 已在 `go.mod`；`payments/providers/alipay` 通过 SDK 生成 `alipay.trade.page.pay` / `alipay.trade.wap.pay` 支付 URL；`service/alipay_webhook.go` 用订单绑定 provider instance 的支付宝公钥验签异步通知，并复用现有金额/币种校验、幂等 audit、billing/subscription fulfill 链路；单测覆盖 signed checkout URL、正常通知履约、无效签名 fail-closed、多商户实例归属和无效 return URL 配置拒绝。真实支付宝沙箱 smoke 仍需外部凭证。）
...
- 状态：已落地（`github.com/wechatpay-apiv3/wechatpay-go` 已在 `go.mod`；`payments/providers/wechat` 通过 SDK 创建 Native / H5 / JSAPI 预支付订单；`service/wechat_webhook.go` 使用微信 APIv3 通知验签与 AES-GCM 解密，并复用现有金额/币种校验、幂等 audit、billing/subscription fulfill 链路；单测覆盖 checkout metadata、签名通知履约、重复通知幂等和本地配置缺失检查。真实微信沙箱 smoke 仍需外部商户凭证。）
...
- 状态：部分落地（`go.opentelemetry.io/otel` / `otel/sdk` / OTLP gRPC exporter 已加入；`platform/otel` 初始化全局 TracerProvider，`app.go` 负责 Shutdown；HTTP 中间件创建 server span 并记录 method/route/status/request_id/response size；配置支持 endpoint、sampling ratio、batch timeout；`scheduler.Schedule`、`payments.HandleWebhook`、`accounts.ProbeAccount` 已补关键 service span、业务 outcome、低敏诊断属性和 `error.type` 错误分类）。
...
- 新增 4 个 smoke：
  ...
  2. `smoke-failover`：已落地为 `make smoke-failover`，创建两个临时 OpenAI-compatible Provider 和本地 mock upstream，验证 primary 503 后自动切到 secondary，并核验 usage attempts、`fallback_from_decision_id`、`fallback_excluded` 和 `srapi_gateway_failover_total`
  3. `smoke-rate-limit`：已落地为 `make smoke-rate-limit`，使用 `rpm_limit=1` 临时 API Key 验证第二个 Gateway 请求返回 429、`rpm_limit_exceeded` 和 `Retry-After`
  4. `smoke-quality-eval`：100 个请求后 QualityEvaluation 出现记录
  5. `rate-limit-bench`：已落地为 `make rate-limit-bench`，对真实 Redis 执行 Allow / AcquireConcurrency / ReleaseConcurrency p99 ≤ 2ms guard

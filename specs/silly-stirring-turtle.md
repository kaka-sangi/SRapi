...
| **策略灰度/dry-run** | 已落地 `scheduler/service/simulator.go` 和 `POST /api/v1/admin/scheduler/simulate`；同一请求 profile 可对比 current/shadow strategy，返回 winner / score delta / cost delta / risk delta，且不创建 SchedulerDecision 或 Lease | 后续可补基于历史 SchedulerDecision 的批量回放、灰度分流百分比和策略对比报表 |
...
- 状态：部分落地（单请求 dry-run + shadow strategy 已完成；批量历史回放和灰度百分比仍待后续 K1.6 扩展）。
- 目标：能用请求 profile 假装用新策略跑一遍，对比 current vs shadow 的选择效果
- 文件：新增 `apps/api/internal/modules/scheduler/service/simulator.go`；新 API `POST /api/v1/admin/scheduler/simulate`
- 要点：使用同一 RequestProfile 和候选集重算两个策略的评分，不真调用上游、不写 SchedulerDecision、不获取 Lease；输出 current/shadow winner 与 final/cost/latency/quality/risk delta
- 验收：服务层和 HTTP 回归证明 shadow decision 不创建 decision/lease，且 cost_saver 可在同一候选集下相对 balanced 改变 winner 并返回 diff
- 依赖：K1.2

...
| **A2 限流中间件** | 已落地 `platform/ratelimit` Redis Lua 原子多维计数，Gateway admission 阶段 enforce API key RPM/TPM 与 User RPM；超限返回 429 + `Retry-After`，并记录 `rpm_limit_exceeded` / `tpm_limit_exceeded` evidence；app 启动时 Redis 可用则注入 limiter，非 release Redis 不可用则降级禁用 | API Key/User 限额字段、Gateway usage evidence、30s account cooldown、Account 层 `rpm_used/rpm_limit` metadata 已存 | 还缺并发 Redis ZSet、账号级 runtime quota counter 写回、scheduler/account-level reject evidence 与 p99 benchmark；核心 API key/user RPM/TPM enforcement 已完成 |
...
- 状态：部分落地（API key/user RPM + API key TPM 已通过 Redis Lua 原子 enforcement；并发 ZSet 与 p99 benchmark 待后续补齐）。
- 目标：API Key 的 RPM/TPM/并发限制在请求入口被真正 enforce
- 文件：新增 `apps/api/internal/platform/ratelimit/{limiter.go,scripts.go}`；改 `runtime_gateway_core.go` 在 admission 后、Scheduler 前检查限额；改 `app.go` / `server.go` 注入 Redis limiter
- 要点：Lua 脚本原子化"判定 + 扣减 + 写回"；Key 模板 `srapi:rl:{kind}:{owner_id}:{window}`；超限返回 429 + `Retry-After`；并发用 Redis ZSet + 过期清理
- 验收：`TestGatewayEnforcesAPIKeyRPMLimit` 证明第二个 Gateway 请求被 429 拒绝；`ratelimit` 单测证明 TPM 超限时 RPM 不发生部分扣减；p99 benchmark 仍待补齐
- 依赖：I2

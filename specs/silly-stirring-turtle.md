...
| 子任务 | 现状关键证据 | 我们预留的资产 | 还缺什么 |
|---|---|---|---|
| **C1 OTel + 结构化 trace** | 已落地 `go.opentelemetry.io/otel` tracing SDK + OTLP gRPC exporter 配置、app 生命周期初始化/Shutdown、HTTP server root span、W3C trace context 提取、slog context handler 自动注入 `request_id` / `trace_id` / `user_id` / `api_key_id`；`/metrics` 已把 Gateway duration 暴露为 histogram bucket（0.1/0.5/1/5s + `+Inf`）；`ObsAlertEvent/ObsSloDefinition` 表完整但无 SLO evaluator 逻辑 | Prometheus 指标可扩展；context 传 request_id/user_id/api_key_id/trace_id；OBS 表设计完整 | 后续补 scheduler/payment/accounts 关键 service span、真实 Jaeger/Tempo collector smoke、SLO breach 检测 worker；若替换为 Prometheus client SDK，需要保持现有指标名和低基数 label 语义 |
...
- 状态：部分落地（`go.opentelemetry.io/otel` / `otel/sdk` / OTLP gRPC exporter 已加入；`platform/otel` 初始化全局 TracerProvider，`app.go` 负责 Shutdown；HTTP 中间件创建 server span 并记录 method/route/status/request_id/response size；配置支持 endpoint、sampling ratio、batch timeout）。

...
- 验收：本地 OTLP gRPC collector smoke 已证明 span 可导出；本地 Jaeger all-in-one/query smoke 已证明 span 可视化链路可查回；Tempo 或生产部署 collector/query 仍需按实际拓扑再跑一次 Gateway 请求确认。
- 依赖：I2

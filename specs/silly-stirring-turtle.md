...
| **A4 跨厂商故障转移** | `gatewayCandidates()` 已遍历所有 provider 的 account，天然支持跨厂商候选；`ScheduleResult.Candidates` 已返回排序候选列表；`SchedulerDecision.fallback_from_decision_id` 已落库并暴露到管理 API；lease update 已按 `(request_id, attempt_no)` 定位，避免 fallback attempt 互相覆盖 | Filter reason 已枚举到 9 类；sticky session、account group 隔离、attempt_no、fallback_excluded reject evidence 已实现 | 还缺 HTTP handler 包裹 retry loop、失败条件白名单和 failover metric；A4.1 调度/持久化基础已完成 |
...
000005 workspaces_and_user_workspace_id     （C3.1）
000006 role_permissions_and_entitlements    （C3.2）
000007 quality_evaluations                  （K1.4）
000008 usage_logs_charged_at_index          （B1.2 性能索引）
```

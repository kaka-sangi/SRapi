...
- 状态：部分落地（单请求 dry-run + shadow strategy、稳定 rollout 预览、真实调度的脱敏 request/candidate snapshot 持久化、`POST /api/v1/admin/scheduler/replay` 批量历史回放 API 已完成；真实流量灰度百分比仍待后续 K1.6 扩展）。

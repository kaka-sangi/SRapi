...
- 验收：本地 OTLP gRPC collector smoke 已证明 span 可导出；真实 Jaeger / Tempo UI 可视化仍需外部 collector 环境再跑一次 Gateway 请求确认。
- 依赖：I2

...
> **当前状态（2026）：Phase 0–5 已交付。** 本文档是 SRapi 的愿景与架构设计蓝本，记录了项目的初始规划与设计取舍。文中描述的端点兼容/转换、调度内核、计费订阅支付、缓存亲和与成本优化、多 Provider 适配与企业级治理等能力均已落地（约 150 个工作包）。当前实现状态以 `STATUS.md` 为准；架构现状参见 `../../docs/requirements/ARCHITECTURE.md` 与 `../FINAL_STATE.md`。下文保留原始规划口吻以记录设计动机，凡仍属于未来路线的部分均显式标注「Roadmap / 尚未实现」。
...
注：商业化能力（多租户 Workspace、权益解析、计费订阅、支付与分销/返佣）原本列为「第一阶段不追求」，现已交付，不再属于非目标——见 §8.9–8.11、§15 Phase 3 及 `STATUS.md`。
...
> 状态：Phase 0–5 均已交付。下文保留原始阶段规划以记录交付顺序与设计动机；各阶段的当前实现状态以 `STATUS.md`、`WORK_PACKAGES.md` 为准。
...
Roadmap / 尚未实现（调度内核长期方向，详见 `../design/SCHEDULING_KERNEL_DESIGN.md` §19）：
...
> 本节记录项目启动时为第一版划定的最小范围，强调先把架构骨架、OpenAPI 流程与调度内核接口打稳。该 MVP 早已超额完成；此处保留作为历史与设计动机参考，当前能力以 `STATUS.md` 为准。

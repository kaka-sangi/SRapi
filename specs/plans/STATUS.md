...
| WP-1320 | AdminOps durable upstream error evidence: `ops_error_logs`, filters, attempt history, resolution workflow, no replay payload storage. |
...
| WP-1236 | ChatGPT Web image edit/variation bridge: `reverse-proxy-chatgpt-web` `/v1/images/edits` and `/v1/images/variations` no longer fall back to OpenAI-compatible multipart `/images/*`; source images upload through ChatGPT Web `/backend-api/files`, edit masks are represented by alpha-composited PNG uploads, and requests run through `/backend-api/f/conversation/prepare` + `/backend-api/f/conversation` with `picture_v2` references before resolving generated image downloads. |
...
- `cd apps/api && go test ./internal/httpserver -run TestAdminOpsErrorLogsListGetAndResolve -count=1`: pass
- `cd apps/web && pnpm --filter @srapi/web typecheck`: pass
...
- WP-1320 moved AdminOps upstream-failure evidence out of usage-derived/local-memory behavior into durable `ops_error_logs` ownership: Ent/PostgreSQL persistence, generated API/SDK contract, admin filters/detail/resolution workflow, frontend error-log panel/detail wiring, and docs/spec governance. The durable row keeps structured attempt/protocol/latency/token/error-owner/upstream-request-id and sanitized attempt-history evidence, but intentionally does not store raw prompts, request bodies, headers, credentials, or retry/replay payloads; sub2api-style replay storage requires a separate redacted replay design.
...
| WP-1320 | completed | AdminOps durable upstream error evidence: `ops_error_logs` Ent/PostgreSQL persistence, store/service wiring, structured attempt/protocol/latency/token/history fields, list/detail/resolution APIs, OpenAPI/SDK, frontend error-log panel/detail migration, docs/spec governance, and an explicit no-raw-request/no-replay-payload policy. |
...
| WP-1236 | completed | ChatGPT Web image edit/variation bridge: after adding `chatgpt2api` as a reference project, `reverse-proxy-chatgpt-web` `/v1/images/edits` and `/v1/images/variations` no longer fall back to OpenAI-compatible multipart `/images/*`; source images upload through ChatGPT Web `/backend-api/files`, edit masks are represented by alpha-composited PNG uploads, and requests run through `/backend-api/f/conversation/prepare` + `/backend-api/f/conversation` with `picture_v2` references before resolving generated image downloads. Adapter regressions cover edit and variation official-client paths and block accidental `/v1/images/edits` / `/v1/images/variations` upstream dispatch. |
| WP-1235 | completed | ChatGPT Web image generation bridge: after adding `chatgpt2api` as a reference project, `reverse-proxy-chatgpt-web` `/v1/images/generations` no longer falls back to OpenAI-compatible `/images/generations`; it prepares/runs ChatGPT Web `/backend-api/f/conversation` with `picture_v2`, extracts generated `file-service://` / `sediment://` image references, resolves download metadata, downloads image bytes through managed egress, and returns OpenAI image response data. Adapter regression covers the official-client path and blocks accidental `/v1/images/generations` upstream dispatch. |

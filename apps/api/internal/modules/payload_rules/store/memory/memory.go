...
// Store is an in-memory implementation of the payload-rule store.
type Store struct {
	mu    sync.Mutex
...
func (s *Store) UpdateRule(ctx context.Context, id int, input contract.UpdateRule) (contract.Rule, error) {
	...
	if input.MatchModel != nil {
		rule.MatchModel = *input.MatchModel
	}
	if input.MatchProtocol != nil {
		rule.MatchProtocol = *input.MatchProtocol
	}
	if input.Params != nil {
		rule.Params = cloneParams(*input.Params)
	}
...
func (s *Store) ListRules(ctx context.Context) ([]contract.Rule, error) {
	...
	for _, rule := range s.rules {
		rule.Params = cloneParams(rule.Params)
		out = append(out, rule)
	}

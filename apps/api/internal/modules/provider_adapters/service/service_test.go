...
func TestGenericReverseProxyAdapterInvokesConfiguredChatUpstream(t *testing.T) {
	...
	if conversationResponseText(resp) != "generic says hi" ||
		...
		resp.Usage.CacheCreation5mTokens != 2 ||
		resp.Usage.CacheCreation1hTokens != 3 {
...
func TestGeminiCompatibleAdapterCapturesModelRateLimitSignal(t *testing.T) {
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusTooManyRequests)
		_, _ = w.Write([]byte(`{"error":{"code":429,"message":"You have exhausted your capacity on this model.","status":"RESOURCE_EXHAUSTED","details":[{"@type":"type.googleapis.com/google.rpc.ErrorInfo","reason":"RATE_LIMIT_EXCEEDED","metadata":{"model":"claude-sonnet-4-5"}},{"@type":"type.googleapis.com/google.rpc.RetryInfo","retryDelay":"2s"}]}}`))
	}))
	defer upstream.Close()
	...
	_, err = svc.InvokeConversation(context.Background(), contract.ConversationRequest{
		RequestID:  "req_gemini_model_rate_signal",
		Model:      "gemini-local",
		InputParts: textParts("hello"),
		Provider:   providercontract.Provider{AdapterType: "gemini-compatible", Protocol: "gemini-compatible"},
		Account:    accountcontract.ProviderAccount{ID: 1, Metadata: map[string]any{"base_url": upstream.URL + "/v1beta"}},
	...
	providerErr := assertProviderError(t, err, "rate_limit", http.StatusTooManyRequests)
	assertQuotaSignal(t, providerErr.QuotaSignals, "google_model_rate_limit_claude_sonnet_4_5", "100", "0", "100", 0)
}
...
func TestGeminiCompatibleAdapterDoesNotCaptureModelCapacityAsRateLimitSignal(t *testing.T) {
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusServiceUnavailable)
		_, _ = w.Write([]byte(`{"error":{"code":503,"message":"No capacity available for model gemini-pro on the server","status":"UNAVAILABLE","details":[{"@type":"type.googleapis.com/google.rpc.ErrorInfo","reason":"MODEL_CAPACITY_EXHAUSTED","metadata":{"model":"gemini-3-pro"}},{"@type":"type.googleapis.com/google.rpc.RetryInfo","retryDelay":"2s"}]}}`))
	}))
	defer upstream.Close()
	...
	_, err = svc.InvokeConversation(context.Background(), contract.ConversationRequest{
		RequestID:  "req_gemini_capacity_no_rate_signal",
		Model:      "gemini-local",
		InputParts: textParts("hello"),
		Provider:   providercontract.Provider{AdapterType: "gemini-compatible", Protocol: "gemini-compatible"},
		Account:    accountcontract.ProviderAccount{ID: 1, Metadata: map[string]any{"base_url": upstream.URL + "/v1beta"}},
	...
	if len(providerErr.QuotaSignals) != 0 {
		t.Fatalf("expected MODEL_CAPACITY_EXHAUSTED to avoid model rate limit signals, got %+v", providerErr.QuotaSignals)
	}
}
...
// TestReverseProxyCodexCLIAdapterCompactConvertsSSEToJSON pins sub2api
// handlePassthroughSSEToJSON parity end-to-end for /v1/responses/compact.
// The upstream Codex backend returns text/event-stream on /compact even
// with Accept: application/json. Codex CLI's compact-task Rust parser
// expects JSON containing an `output[]` with a message item whose
// content[].text holds the summary. Without the conversion+reconstruction
// the parser explodes with "missing field `text` at line 1 column N".
//
// The adapter now (1) extracts the terminal response.completed event's
// `response` field, (2) reconstructs output[] from
// response.output_text.delta events when the terminal arrived with an
// empty output array, and (3) flips Content-Type to application/json
// so the gateway forwards JSON to codex CLI.
func TestReverseProxyCodexCLIAdapterCompactConvertsSSEToJSON(t *testing.T) {
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/event-stream")
		_, _ = w.Write([]byte(
			"data: {\"type\":\"response.output_text.delta\",\"delta\":\"summary \"}\n\n" +
				"data: {\"type\":\"response.output_text.delta\",\"delta\":\"text.\"}\n\n" +
				"data: {\"type\":\"response.completed\",\"response\":{\"id\":\"cmp_e2e\",\"object\":\"response.compaction\",\"output\":[],\"input_tokens\":7,\"output_tokens\":1}}\n\n" +
				"data: [DONE]\n\n",
		))
	}))
	defer upstream.Close()
	...
	// Raw must be JSON (no SSE markers) with the reconstructed output[].
	...
	if payload.ID != "cmp_e2e" || payload.Object != "response.compaction" {
		t.Fatalf("compact metadata lost, got %+v", payload)
	}
	if len(payload.Output) != 1 || payload.Output[0].Type != "message" {
		t.Fatalf("output[] must contain a message item, got %+v", payload.Output)
	}
	if len(payload.Output[0].Content) != 1 || payload.Output[0].Content[0].Text != "summary text." {
		t.Fatalf("output[].content[].text must reconstruct delta text, got %+v", payload.Output[0].Content)
	}
	// Headers must be flipped to application/json so the gateway emits
	// the correct content-type to codex CLI.
...
func TestReverseProxyCodexCLIAdapterBridgesImageEditToResponses(t *testing.T) {
	runtime := capturingRuntime{
		response: reverseproxycontract.Response{
			StatusCode: http.StatusOK,
			Body: []byte(
				"data: {\"type\":\"response.output_item.done\",\"output_index\":0,\"item\":{\"id\":\"ig_1\",\"type\":\"image_generation_call\",\"status\":\"completed\",\"result\":\"ZWRpdGVk\",\"revised_prompt\":\"replace background, revised\",\"output_format\":\"webp\"}}\n\n" +
					"data: {\"type\":\"response.completed\",\"response\":{\"id\":\"resp_img_edit\",\"status\":\"completed\",\"model\":\"gpt-5.4\",\"usage\":{\"input_tokens\":16,\"output_tokens\":8,\"output_tokens_details\":{\"image_tokens\":6}},\"output\":[]}}\n\n" +
					"data: [DONE]\n\n",
			),
			Headers: http.Header{
	...
	resp, err := svc.InvokeImageEdit(context.Background(), contract.ImageEditRequest{
		RequestID:      "req_codex_image_edit",
		Model:          "gpt-image-2",
	...
	if err != nil {
		t.Fatalf("invoke codex image edit bridge: %v", err)
	}
	if runtime.request.Method != http.MethodPost || runtime.request.URL != "https://upstream.example/backend-api/codex/responses" {
		t.Fatalf("unexpected codex image edit runtime request: %+v", runtime.request)
	}
	if runtime.request.Headers.Get("Accept") != "text/event-stream" ||
		...
		t.Fatalf("unexpected codex image edit headers: %+v", runtime.request.Headers)
	}
	var payload struct {
		Model  string `json:"model"`
		Stream bool   `json:"stream"`
	...
	if err := json.Unmarshal(runtime.request.Body, &payload); err != nil {
		t.Fatalf("decode codex image edit payload: %v", err)
	}
	if payload.Model != "gpt-5.4" || !payload.Stream || len(payload.Input) != 1 || payload.Input[0].Role != "user" || len(payload.Input[0].Content) != 2 {
		t.Fatalf("unexpected codex image edit payload: %+v", payload)
	}
	if payload.Input[0].Content[0].Type != "input_text" || payload.Input[0].Content[0].Text != "replace background" ||
		payload.Input[0].Content[1].Type != "input_image" ||
		payload.Input[0].Content[1].ImageURL != "data:image/png;base64,UE5HLXNvdXJjZQ==" {
		t.Fatalf("unexpected codex image edit input: %+v", payload.Input[0].Content)
	}
	if len(payload.Tools) != 1 ||
		payload.Tools[0].Type != "image_generation" ||
		...
		payload.Tools[0].Background != "transparent" ||
		payload.Tools[0].OutputFormat != "webp" ||
		payload.Tools[0].OutputCompression != 80 ||
		payload.Tools[0].PartialImages != 2 ||
		payload.Tools[0].InputFidelity != "high" ||
		payload.Tools[0].InputImageMask.ImageURL != "data:image/png;base64,UE5HLW1hc2s=" {
		t.Fatalf("unexpected codex image edit tool: %+v", payload.Tools)
	}
	if len(resp.Data) != 1 || resp.Data[0].Base64JSON != "ZWRpdGVk" || resp.Data[0].RevisedPrompt != "replace background, revised" || resp.Model != "gpt-5.4" || resp.Usage.InputTokens != 16 || resp.Usage.OutputTokens != 8 || resp.Usage.ImageOutputTokens != 6 {
		t.Fatalf("unexpected codex image edit response: %+v", resp)
	}
	if len(resp.QuotaSignals) != 1 {
...
func TestReverseProxyCodexCLIAdapterImageEditEstimatesUsageWithInputImages(t *testing.T) {
	runtime := capturingRuntime{
		response: reverseproxycontract.Response{
			StatusCode: http.StatusOK,
			Body: []byte(
				"data: {\"type\":\"response.completed\",\"response\":{\"id\":\"resp_img_edit\",\"status\":\"completed\",\"output\":[{\"id\":\"ig_1\",\"type\":\"image_generation_call\",\"status\":\"completed\",\"result\":\"ZWRpdGVk\",\"output_format\":\"png\"}]}}\n\n" +
					"data: [DONE]\n\n",
			),
		},
	}
	...
	resp, err := svc.InvokeImageEdit(context.Background(), contract.ImageEditRequest{
		RequestID:      "req_codex_image_edit_usage",
		Model:          "gpt-image-2",
		Prompt:         "replace background",
		Images:         []contract.ImageInput{{FileName: "source.png", ContentType: "image/png", Bytes: bytes.Repeat([]byte("a"), 3072)}},
		Mask:           &contract.ImageInput{FileName: "mask.png", ContentType: "image/png", Bytes: bytes.Repeat([]byte("b"), 2048)},
		ResponseFormat: "b64_json",
		Provider: providercontract.Provider{
			AdapterType: "reverse-proxy-codex-cli",
			Protocol:    "openai-compatible",
	...
	if !resp.Usage.Estimated || resp.Usage.InputTokens < 5 {
		t.Fatalf("expected estimated usage to include source image and mask bytes, got %+v", resp.Usage)
	}
}

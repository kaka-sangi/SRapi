...
1. **收集信息** — 向用户索要：
   ...
   - **不要**向用户索要 `client_id` — 系统会自动使用正确的 ChatGPT Web client_id（`app_2SKx67EdpoN0G6j64rFvigXD`）。如果 credential 里存了错误的 client_id（比如 Codex CLI 的），刷新会报 "invalid client id"。
...
3. **逐个创建账号** — refresh_token 很长，批量接口可能超出大小限制，使用单条创建：
   ...
   ```
   **注意：不要在 credentials 里放 client_id**。系统检测到 `upstream_client: "chatgpt_web"` 后会自动使用内置的 ChatGPT Web client_id。手动传入 client_id 会覆盖系统默认值，导致 "invalid client id" 错误。

...
func TestAdminAccountAuthMethodAllowlistScopesRuntimeClass(t *testing.T) {
	...
	// Antigravity-style allowlist accepts oauth_refresh and rejects unsupported
	// runtime classes that are not preset auth methods.
	...
	rec = adminAccountMutation(t, handler, sessionCookie, csrf, http.MethodPost, "/api/v1/admin/accounts",
		`{"provider_id":"`+string(antigravity.Data.Id)+`","name":"antigravity-unsupported","runtime_class":"unsupported_runtime","upstream_client":"antigravity_desktop","credential":{"access_token":"token"},"metadata":{"base_url":"https://example.test","project_id":"p1"},"status":"active"}`)
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected 400 rejecting unsupported runtime on antigravity-scoped provider, got %d body=%s", rec.Code, rec.Body.String())
	}

...
func TestMetricsExposeBaselineSRapiSignals(t *testing.T) {
	...
	for _, metric := range []string{
		...
		"srapi_scheduler_candidate_count_bucket",
...
func TestGatewayGeminiRateLimitUsesGoogleEnvelopeAndRetryAfter(t *testing.T) {
	...
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"candidates":[{"content":{"role":"model","parts":[{"text":"gemini ok"}]}}],"usageMetadata":{"promptTokenCount":4,"candidatesTokenCount":2,"totalTokenCount":6}}`))
	}))
	defer upstream.Close()

	handler := New(config.Load(), nil, WithRateLimiter(limiter))
	loginResp, sessionCookie := mustLoginAdmin(t, handler)
	mustCreateNativeGeminiGatewayTargetWithNames(t, handler, sessionCookie, loginResp.Data.CsrfToken, upstream.URL, "gemini-rate-limit-provider", "gemini-route-model", "gemini-route-upstream", "gemini-rate-limit-account")
...
func TestGatewayGeminiGenerateContentRequiresGeminiEndpointCapability(t *testing.T) {
	var (
		mu    sync.Mutex
	...
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		mu.Unlock()
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"candidates":[{"content":{"role":"model","parts":[{"text":"gemini upstream response"}]}}],"usageMetadata":{"promptTokenCount":6,"candidatesTokenCount":8,"totalTokenCount":14}}`))
	}))
	defer upstream.Close()

	handler := New(config.Load(), nil)
	loginResp, sessionCookie := mustLoginAdmin(t, handler)
	openAIProvider := mustCreateProvider(t, handler, sessionCookie, loginResp.Data.CsrfToken, `{"name":"gemini-openai-wrong-provider","display_name":"Gemini OpenAI Wrong Provider","adapter_type":"openai-compatible","protocol":"openai-compatible","status":"active"}`)
	providerResp, modelResp, accountResp := mustCreateNativeGeminiGatewayTargetWithNames(t, handler, sessionCookie, loginResp.Data.CsrfToken, upstream.URL, "gemini-endpoint-provider", "gemini-route-model", "gemini-route-upstream", "gemini-endpoint-account")
	mustCreateMapping(t, handler, sessionCookie, loginResp.Data.CsrfToken, string(modelResp.Data.Id), `{"provider_id":"`+string(openAIProvider.Data.Id)+`","upstream_model_name":"bad-openai-upstream","status":"active"}`)
	openAIAccount := mustCreateAccount(t, handler, sessionCookie, loginResp.Data.CsrfToken, `{"provider_id":"`+string(openAIProvider.Data.Id)+`","name":"gemini-openai-wrong-account","runtime_class":"api_key","credential":{"api_key":"bad-openai-secret"},"metadata":{"base_url":"https://bad.example.invalid/v1","quality_score":1,"health_score":1,"latency_p95_ms":1},"status":"active"}`)
	_, apiKey := mustCreateGatewayAPIKey(t, handler, sessionCookie, loginResp.Data.CsrfToken)
	...
	call := gotCalls[0]
	if call.Path != "/v1beta/models/gemini-route-upstream:generateContent" || call.APIKey != "native-gemini-secret" || call.MaxTokens != 32 {
		t.Fatalf("unexpected native Gemini upstream call: %+v", call)
	}
	if call.SystemText != "be brief" || len(call.Contents) != 1 || call.Contents[0].Role != "user" || call.Contents[0].Parts[0].Text != "gemini route prompt" {
		t.Fatalf("expected Gemini payload forwarded upstream, got %+v", call)
	}

	decisionsReq := httptest.NewRequest(http.MethodGet, "/api/v1/admin/scheduler/decisions?model=gemini-route-model", nil)
	decisionsReq.AddCookie(sessionCookie)
	decisionsRec := httptest.NewRecorder()
	handler.ServeHTTP(decisionsRec, decisionsReq)
	if decisionsRec.Code != http.StatusOK {
	...
	if decision.SelectedAccountId == nil || *decision.SelectedAccountId != string(accountResp.Data.Id) {
		t.Fatalf("expected native Gemini account %s to be selected, got %+v", accountResp.Data.Id, decision)
	}
	if decision.RejectReasons["account_"+string(openAIAccount.Data.Id)] != "capability_mismatch:gemini_generate_content" {
		t.Fatalf("expected OpenAI-compatible candidate to be rejected by Gemini endpoint capability, got %+v", decision.RejectReasons)
	}
	...
	usage := usageResp.Data[0]
	if !usage.Success || usage.SourceProtocol != "gemini-compatible" || usage.SourceEndpoint != "/v1beta/models/gemini-route-model:generateContent" || usage.TargetProtocol == nil || *usage.TargetProtocol != "gemini-compatible" || usage.TotalTokens != 14 || usage.UsageEstimated {
...
func TestGatewayGeminiGenerateContentRecordsExplicitZeroUsage(t *testing.T) {
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/v1beta/models/gemini-native-upstream:generateContent" {
			t.Fatalf("unexpected native gemini path %s", r.URL.Path)
		}
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"candidates":[{"content":{"role":"model","parts":[{"text":"zero token response"}]}}],"usageMetadata":{"promptTokenCount":0,"candidatesTokenCount":0,"totalTokenCount":0}}`))
	}))
	defer upstream.Close()
	...
	body := `{"contents":[{"role":"user","parts":[{"text":"return explicit zero usage"}]}]}`
	rec := mustGatewayRequest(t, handler, apiKey, http.MethodPost, "/v1beta/models/native-gemini-route-model:generateContent", body)
	var resp apiopenapi.GeminiGenerateContentResponse
	if err := json.NewDecoder(rec.Body).Decode(&resp); err != nil {
		t.Fatalf("decode gemini response: %v", err)
	}
	if resp.UsageMetadata == nil || resp.UsageMetadata.TotalTokenCount == nil || *resp.UsageMetadata.TotalTokenCount != 0 {
		t.Fatalf("expected explicit zero usage metadata, got %+v", resp.UsageMetadata)
	}
	...
	if !usage.Success ||
		...
		usage.SourceEndpoint != "/v1beta/models/native-gemini-route-model:generateContent" ||
		usage.TargetProtocol == nil ||
		*usage.TargetProtocol != "gemini-compatible" {
		t.Fatalf("unexpected explicit zero usage record: %+v", usage)
	}
	if usage.ProviderId == nil || *usage.ProviderId != providerResp.Data.Id || usage.AccountId == nil || *usage.AccountId != accountResp.Data.Id || usage.Model != modelResp.Data.CanonicalName {
		t.Fatalf("expected provider/account/model evidence, got %+v", usage)
	}
...
func TestGatewayGeminiCountTokensRequiresGeminiEndpointCapability(t *testing.T) {
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/v1beta/models/gemini-native-upstream:countTokens" {
			t.Fatalf("unexpected native gemini count path %s", r.URL.Path)
		}
		w.Header().Set("Content-Type", "application/json")
	...
	anthropicProvider := mustCreateProvider(t, handler, sessionCookie, csrf, `{"name":"gemini-count-anthropic-wrong","display_name":"Gemini Count Anthropic Wrong","adapter_type":"anthropic-compatible","protocol":"anthropic-compatible","status":"active"}`)
	geminiProvider, modelResp, geminiAccount := mustCreateNativeGeminiGatewayTargetWithNames(t, handler, sessionCookie, csrf, upstream.URL, "gemini-count-right-provider", "gemini-count-filter-model", "gemini-native-upstream", "gemini-count-right-account")
	mustCreateMapping(t, handler, sessionCookie, csrf, string(modelResp.Data.Id), `{"provider_id":"`+string(anthropicProvider.Data.Id)+`","upstream_model_name":"claude-count-upstream","status":"active"}`)
	anthropicAccount := mustCreateAccount(t, handler, sessionCookie, csrf, `{"provider_id":"`+string(anthropicProvider.Data.Id)+`","name":"gemini-count-anthropic-wrong-account","runtime_class":"api_key","credential":{"api_key":"anthropic-count-secret"},"metadata":{"base_url":"https://api.anthropic.com/v1","quality_score":1,"health_score":1,"latency_p95_ms":1},"status":"active"}`)
	_, apiKey := mustCreateGatewayAPIKey(t, handler, sessionCookie, csrf)

	rec := mustGatewayRequest(t, handler, apiKey, http.MethodPost, "/v1beta/models/gemini-count-filter-model:countTokens", `{"contents":[{"role":"user","parts":[{"text":"count gemini only"}]}]}`)
	var resp apiopenapi.GeminiCountTokensResponse
	if err := json.NewDecoder(rec.Body).Decode(&resp); err != nil {
		t.Fatalf("decode gemini count response: %v", err)
	}
	...
	decisionsReq := httptest.NewRequest(http.MethodGet, "/api/v1/admin/scheduler/decisions?model=gemini-count-filter-model", nil)
	decisionsReq.AddCookie(sessionCookie)
	decisionsRec := httptest.NewRecorder()
	handler.ServeHTTP(decisionsRec, decisionsReq)
	if decisionsRec.Code != http.StatusOK {
	...
	decision := decisionsResp.Data[0]
	if decision.SelectedProviderId == nil || *decision.SelectedProviderId != string(geminiProvider.Data.Id) ||
		decision.SelectedAccountId == nil || *decision.SelectedAccountId != string(geminiAccount.Data.Id) {
		t.Fatalf("expected Gemini provider/account to be selected, got %+v", decision)
	}
	if decision.RejectReasons["account_"+string(anthropicAccount.Data.Id)] != "capability_mismatch:gemini_count_tokens" {
		t.Fatalf("expected Anthropic candidate to be rejected by Gemini count capability, got %+v", decision.RejectReasons)
	}
}
...
func TestGatewayAnthropicCountTokensRejectsGeminiTokenCountingCandidate(t *testing.T) {
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/v1/messages/count_tokens" {
			t.Fatalf("unexpected Anthropic count path %s", r.URL.Path)
		}
		w.Header().Set("Content-Type", "application/json")
	...
	loginResp, sessionCookie := mustLoginAdmin(t, handler)
	csrf := loginResp.Data.CsrfToken
	geminiProvider := mustCreateProvider(t, handler, sessionCookie, csrf, `{"name":"anthropic-count-gemini-wrong","display_name":"Anthropic Count Gemini Wrong","adapter_type":"gemini-compatible","protocol":"gemini-compatible","status":"active"}`)
	anthropicProvider := mustCreateProvider(t, handler, sessionCookie, csrf, `{"name":"anthropic-count-right-provider","display_name":"Anthropic Count Right","adapter_type":"anthropic-compatible","protocol":"anthropic-compatible","status":"active"}`)
	modelResp := mustCreateModel(t, handler, sessionCookie, csrf, `{"canonical_name":"anthropic-count-filter-model","display_name":"Anthropic Count Filter Model","status":"active","capabilities":[{"key":"token_counting","level":"required","status":"stable","version":"v1"}]}`)
	mustCreateMapping(t, handler, sessionCookie, csrf, string(modelResp.Data.Id), `{"provider_id":"`+string(geminiProvider.Data.Id)+`","upstream_model_name":"gemini-count-upstream","status":"active"}`)
	geminiAccount := mustCreateAccount(t, handler, sessionCookie, csrf, `{"provider_id":"`+string(geminiProvider.Data.Id)+`","name":"anthropic-count-gemini-wrong-account","runtime_class":"api_key","credential":{"api_key":"gemini-count-secret"},"metadata":{"base_url":"https://generativelanguage.googleapis.com/v1beta","quality_score":1,"health_score":1,"latency_p95_ms":1},"status":"active"}`)
	mustCreateMapping(t, handler, sessionCookie, csrf, string(modelResp.Data.Id), `{"provider_id":"`+string(anthropicProvider.Data.Id)+`","upstream_model_name":"claude-count-upstream","status":"active"}`)
	anthropicAccount := mustCreateAccount(t, handler, sessionCookie, csrf, `{"provider_id":"`+string(anthropicProvider.Data.Id)+`","name":"anthropic-count-right-account","runtime_class":"api_key","credential":{"api_key":"anthropic-count-secret"},"metadata":{"base_url":"`+upstream.URL+`/v1","quality_score":0.1,"health_score":0.1,"latency_p95_ms":1000},"status":"active"}`)
	_, apiKey := mustCreateGatewayAPIKey(t, handler, sessionCookie, csrf)

	rec := mustGatewayRequest(t, handler, apiKey, http.MethodPost, "/v1/messages/count_tokens", `{"model":"anthropic-count-filter-model","messages":[{"role":"user","content":"count anthropic only"}]}`)
	var resp apiopenapi.AnthropicCountTokensResponse
	if err := json.NewDecoder(rec.Body).Decode(&resp); err != nil {
		t.Fatalf("decode Anthropic count response: %v", err)
	}
	...
	decisionsReq := httptest.NewRequest(http.MethodGet, "/api/v1/admin/scheduler/decisions?model=anthropic-count-filter-model", nil)
	decisionsReq.AddCookie(sessionCookie)
	decisionsRec := httptest.NewRecorder()
	handler.ServeHTTP(decisionsRec, decisionsReq)
	if decisionsRec.Code != http.StatusOK {
	...
	decision := decisionsResp.Data[0]
	if decision.SelectedProviderId == nil || *decision.SelectedProviderId != string(anthropicProvider.Data.Id) ||
		decision.SelectedAccountId == nil || *decision.SelectedAccountId != string(anthropicAccount.Data.Id) {
		t.Fatalf("expected Anthropic provider/account to be selected, got %+v", decision)
	}
	if decision.RejectReasons["account_"+string(geminiAccount.Data.Id)] != "capability_mismatch:anthropic_count_tokens" {
		t.Fatalf("expected Gemini candidate to be rejected by Anthropic count capability, got %+v", decision.RejectReasons)
	}
}
...
func TestGatewayGeminiStreamGenerateContentEmitsGeminiSSE(t *testing.T) {
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/event-stream")
		_, _ = w.Write([]byte("data: {\"candidates\":[{\"content\":{\"parts\":[{\"text\":\"gemini stream\"}]}}]}\n\n"))
		_, _ = w.Write([]byte("data: {\"candidates\":[{\"content\":{\"parts\":[{\"text\":\" response\"}]}}]}\n\n"))
		_, _ = w.Write([]byte("data: {\"candidates\":[],\"usageMetadata\":{\"promptTokenCount\":3,\"candidatesTokenCount\":4,\"totalTokenCount\":7}}\n\n"))
	...
	handler := New(config.Load(), nil)
	loginResp, sessionCookie := mustLoginAdmin(t, handler)
	mustCreateNativeGeminiGatewayTargetWithNames(t, handler, sessionCookie, loginResp.Data.CsrfToken, upstream.URL, "gemini-stream-provider", "gemini-route-model", "gemini-route-upstream", "gemini-stream-account")
	_, apiKey := mustCreateGatewayAPIKey(t, handler, sessionCookie, loginResp.Data.CsrfToken)
...
func TestGatewayGeminiProviderErrorsUseGoogleStyleEnvelope(t *testing.T) {
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"error":{"code":429,"status":"RESOURCE_EXHAUSTED","message":"rate limit: slow down"}}`))
	}))
	defer upstream.Close()

	handler := New(config.Load(), nil)
	loginResp, sessionCookie := mustLoginAdmin(t, handler)
	mustCreateNativeGeminiGatewayTargetWithNames(t, handler, sessionCookie, loginResp.Data.CsrfToken, upstream.URL, "gemini-error-provider", "gemini-route-model", "gemini-route-upstream", "gemini-error-account")
	_, apiKey := mustCreateGatewayAPIKey(t, handler, sessionCookie, loginResp.Data.CsrfToken)
...
func TestGatewayResponsesInputItemsAliasReplaysRawUpstreamJSON(t *testing.T) {
	...
	// Create a no-base_url account at priority 100 that deterministically fails
	// with configuration_error. The real account at priority 200 succeeds on
	// failover — exercising the input_items failover path explicitly.
	mustCreateAccount(t, handler, sessionCookie, loginResp.Data.CsrfToken, `{"provider_id":"`+string(openaiProvider.Id)+`","name":"input-items-no-base-url","runtime_class":"api_key","credential":{"api_key":"will-fail"},"status":"active","priority":100}`)
...
func TestGatewayImageGenerationRouteTargetsOpenAICompatibleUpstream(t *testing.T) {
	...
	handler := New(config.Load(), nil)
	loginResp, sessionCookie := mustLoginAdmin(t, handler)
	providerResp := mustCreateProvider(t, handler, sessionCookie, loginResp.Data.CsrfToken, `{"name":"wp290-openai","display_name":"WP290 OpenAI","adapter_type":"openai-compatible","protocol":"openai-compatible","status":"active","capabilities":{"image_generations":true}}`)
	modelResp := mustCreateModel(t, handler, sessionCookie, loginResp.Data.CsrfToken, `{"canonical_name":"wp290-image-model","display_name":"WP290 Image Model","status":"active","capabilities":[{"key":"image_generations","level":"required","status":"stable","version":"v1"}]}`)
...
func TestGatewayImageGenerationStreamReturnsSSE(t *testing.T) {
	...
	handler := New(config.Load(), nil)
	loginResp, sessionCookie := mustLoginAdmin(t, handler)
	providerResp := mustCreateProvider(t, handler, sessionCookie, loginResp.Data.CsrfToken, `{"name":"wp530-openai","display_name":"WP530 OpenAI","adapter_type":"openai-compatible","protocol":"openai-compatible","status":"active","capabilities":{"image_generations":true}}`)
	modelResp := mustCreateModel(t, handler, sessionCookie, loginResp.Data.CsrfToken, `{"canonical_name":"wp530-image-generation-stream-model","display_name":"WP530 Image Generation Stream Model","status":"active","capabilities":[{"key":"image_generations","level":"required","status":"stable","version":"v1"}]}`)
...
func TestGatewayImageGenerationPoolModeRetriesThenFailsOver(t *testing.T) {
	...
	handler := New(config.Load(), nil)
	loginResp, sessionCookie := mustLoginAdmin(t, handler)
	modelResp := mustCreateModel(t, handler, sessionCookie, loginResp.Data.CsrfToken, `{"canonical_name":"image-direct-failover-model","display_name":"Image Direct Failover Model","status":"active","capabilities":[{"key":"image_generations","level":"required","status":"stable","version":"v1"}]}`)
	primaryProvider := mustCreateProvider(t, handler, sessionCookie, loginResp.Data.CsrfToken, `{"name":"image-direct-primary-provider","display_name":"Image Direct Primary","adapter_type":"openai-compatible","protocol":"openai-compatible","status":"active","capabilities":{"image_generations":true}}`)
	secondaryProvider := mustCreateProvider(t, handler, sessionCookie, loginResp.Data.CsrfToken, `{"name":"image-direct-secondary-provider","display_name":"Image Direct Secondary","adapter_type":"openai-compatible","protocol":"openai-compatible","status":"active","capabilities":{"image_generations":true}}`)
...
func TestGatewayImageGenerationAliasForcesProviderContext(t *testing.T) {
	...
	loginResp, sessionCookie := mustLoginAdmin(t, handler)
	openaiProvider := mustFindProviderByName(t, handler, sessionCookie, "openai-compatible")
	fallbackProvider := mustCreateProvider(t, handler, sessionCookie, loginResp.Data.CsrfToken, `{"name":"image-fallback-provider","display_name":"Image Fallback","adapter_type":"openai-compatible","protocol":"openai-compatible","status":"active","capabilities":{"image_generations":true}}`)
	modelResp := mustCreateModel(t, handler, sessionCookie, loginResp.Data.CsrfToken, `{"canonical_name":"wp290-alias-image-model","display_name":"WP290 Alias Image Model","status":"active","capabilities":[{"key":"image_generations","level":"required","status":"stable","version":"v1"}]}`)
...
func TestGatewayImageEditRouteTargetsOpenAICompatibleUpstream(t *testing.T) {
	...
	handler := New(config.Load(), nil)
	loginResp, sessionCookie := mustLoginAdmin(t, handler)
	providerResp := mustCreateProvider(t, handler, sessionCookie, loginResp.Data.CsrfToken, `{"name":"wp480-openai","display_name":"WP480 OpenAI","adapter_type":"openai-compatible","protocol":"openai-compatible","status":"active","capabilities":{"image_edits":true}}`)
	modelResp := mustCreateModel(t, handler, sessionCookie, loginResp.Data.CsrfToken, `{"canonical_name":"wp480-image-edit-model","display_name":"WP480 Image Edit Model","status":"active","capabilities":[{"key":"image_edits","level":"required","status":"stable","version":"v1"}]}`)
...
func TestGatewayImageEditAcceptsJSONImageReferences(t *testing.T) {
	...
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if maskFile, maskHeader, err := r.FormFile("mask"); err == nil {
			maskBytes, readErr := io.ReadAll(maskFile)
			_ = maskFile.Close()
			if readErr != nil {
				t.Fatalf("read upstream mask: %v", readErr)
			}
			call.MaskFilename = maskHeader.Filename
			call.MaskContentType = maskHeader.Header.Get("Content-Type")
			call.Mask = string(maskBytes)
		} else if !errors.Is(err, http.ErrMissingFile) {
			t.Fatalf("open upstream mask: %v", err)
		}
		mu.Lock()
		calls = append(calls, call)
		mu.Unlock()
	...
	handler := New(config.Load(), nil)
	loginResp, sessionCookie := mustLoginAdmin(t, handler)
	providerResp := mustCreateProvider(t, handler, sessionCookie, loginResp.Data.CsrfToken, `{"name":"wp510-openai","display_name":"WP510 OpenAI","adapter_type":"openai-compatible","protocol":"openai-compatible","status":"active","capabilities":{"image_edits":true}}`)
	modelResp := mustCreateModel(t, handler, sessionCookie, loginResp.Data.CsrfToken, `{"canonical_name":"wp510-image-edit-json-model","display_name":"WP510 Image Edit JSON Model","status":"active","capabilities":[{"key":"image_edits","level":"required","status":"stable","version":"v1"}]}`)
	...
	jsonBody := `{"model":"wp510-image-edit-json-model","prompt":"combine references","n":1,"size":"1024x1536","quality":"high","response_format":"b64_json","output_format":"webp","output_compression":88,"background":"transparent","images":[{"image_url":"data:image/png;base64,UE5HLWpzb24="},{"b64_json":"SlBFRy1qc29u","mime_type":"image/jpeg","filename":"two.jpg"}],"mask":{"image_url":"data:image/png;base64,TUFTSy1qc29u","filename":"mask.png"}}`
	...
	if got.Mask != "MASK-json" || got.MaskFilename != "mask.png" || got.MaskContentType != "image/png" {
		t.Fatalf("unexpected upstream image edit json mask: %+v", got)
	}
}
...
func TestGatewayImageEditStreamReturnsSSE(t *testing.T) {
	...
	handler := New(config.Load(), nil)
	loginResp, sessionCookie := mustLoginAdmin(t, handler)
	providerResp := mustCreateProvider(t, handler, sessionCookie, loginResp.Data.CsrfToken, `{"name":"wp520-openai","display_name":"WP520 OpenAI","adapter_type":"openai-compatible","protocol":"openai-compatible","status":"active","capabilities":{"image_edits":true}}`)
	modelResp := mustCreateModel(t, handler, sessionCookie, loginResp.Data.CsrfToken, `{"canonical_name":"wp520-image-edit-stream-model","display_name":"WP520 Image Edit Stream Model","status":"active","capabilities":[{"key":"image_edits","level":"required","status":"stable","version":"v1"}]}`)
...
func TestGatewayImageEditJSONStreamReturnsSSE(t *testing.T) {
	...
	handler := New(config.Load(), nil)
	loginResp, sessionCookie := mustLoginAdmin(t, handler)
	providerResp := mustCreateProvider(t, handler, sessionCookie, loginResp.Data.CsrfToken, `{"name":"wp520-json-openai","display_name":"WP520 JSON OpenAI","adapter_type":"openai-compatible","protocol":"openai-compatible","status":"active","capabilities":{"image_edits":true}}`)
	modelResp := mustCreateModel(t, handler, sessionCookie, loginResp.Data.CsrfToken, `{"canonical_name":"wp520-image-edit-json-stream-model","display_name":"WP520 Image Edit JSON Stream Model","status":"active","capabilities":[{"key":"image_edits","level":"required","status":"stable","version":"v1"}]}`)
...
func TestGatewayImageEditRejectsUnsupportedJSONReferences(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		...
		"remote_url": {
			body:    `{"model":"gpt-image-1","prompt":"edit","images":[{"image_url":"https://example.com/image.png"}]}`,
			message: "remote image URLs are not supported",
		},
		"file_id": {
			body:    `{"model":"gpt-image-1","prompt":"edit","images":[{"file_id":"file-abc123"}]}`,
			message: "file_id image references are not supported",
		},
		"mask_remote_url": {
			body:    `{"model":"gpt-image-1","prompt":"edit","images":[{"image_url":"data:image/png;base64,UE5H"}],"mask":{"image_url":"https://example.com/mask.png"}}`,
			message: "remote image URLs are not supported",
		},
		"mask_file_id": {
			body:    `{"model":"gpt-image-1","prompt":"edit","images":[{"image_url":"data:image/png;base64,UE5H"}],"mask":{"file_id":"file-mask123"}}`,
			message: "file_id image references are not supported",
		},
	} {
		t.Run(name, func(t *testing.T) {
			req := httptest.NewRequest(http.MethodPost, "/v1/images/edits", strings.NewReader(tc.body))
			req.Header.Set("Content-Type", "application/json")
			req.Header.Set("Authorization", "Bearer "+apiKey)
			rec := httptest.NewRecorder()
			handler.ServeHTTP(rec, req)
...
func TestGatewayImageEditAliasForcesProviderContext(t *testing.T) {
	...
	loginResp, sessionCookie := mustLoginAdmin(t, handler)
	openaiProvider := mustFindProviderByName(t, handler, sessionCookie, "openai-compatible")
	fallbackProvider := mustCreateProvider(t, handler, sessionCookie, loginResp.Data.CsrfToken, `{"name":"image-edit-fallback-provider","display_name":"Image Edit Fallback","adapter_type":"openai-compatible","protocol":"openai-compatible","status":"active","capabilities":{"image_edits":true}}`)
	modelResp := mustCreateModel(t, handler, sessionCookie, loginResp.Data.CsrfToken, `{"canonical_name":"wp480-alias-image-edit-model","display_name":"WP480 Alias Image Edit Model","status":"active","capabilities":[{"key":"image_edits","level":"required","status":"stable","version":"v1"}]}`)
...
func TestGatewayImageVariationRouteTargetsOpenAICompatibleUpstream(t *testing.T) {
	...
	handler := New(config.Load(), nil)
	loginResp, sessionCookie := mustLoginAdmin(t, handler)
	providerResp := mustCreateProvider(t, handler, sessionCookie, loginResp.Data.CsrfToken, `{"name":"wp490-openai","display_name":"WP490 OpenAI","adapter_type":"openai-compatible","protocol":"openai-compatible","status":"active","capabilities":{"image_variations":true}}`)
	modelResp := mustCreateModel(t, handler, sessionCookie, loginResp.Data.CsrfToken, `{"canonical_name":"wp490-image-variation-model","display_name":"WP490 Image Variation Model","status":"active","capabilities":[{"key":"image_variations","level":"required","status":"stable","version":"v1"}]}`)
...
func TestGatewayImageVariationAliasForcesProviderContext(t *testing.T) {
	...
	loginResp, sessionCookie := mustLoginAdmin(t, handler)
	openaiProvider := mustFindProviderByName(t, handler, sessionCookie, "openai-compatible")
	fallbackProvider := mustCreateProvider(t, handler, sessionCookie, loginResp.Data.CsrfToken, `{"name":"image-variation-fallback-provider","display_name":"Image Variation Fallback","adapter_type":"openai-compatible","protocol":"openai-compatible","status":"active","capabilities":{"image_variations":true}}`)
	modelResp := mustCreateModel(t, handler, sessionCookie, loginResp.Data.CsrfToken, `{"canonical_name":"wp490-alias-image-variation-model","display_name":"WP490 Alias Image Variation Model","status":"active","capabilities":[{"key":"image_variations","level":"required","status":"stable","version":"v1"}]}`)
...
func TestGatewayEnforcesAccountRPMOnDirectDispatchRouteWithRedisCounter(t *testing.T) {
	...
	handler := New(config.Load(), nil, WithRateLimiter(limiter))
	loginResp, sessionCookie := mustLoginAdmin(t, handler)
	providerResp := mustCreateProvider(t, handler, sessionCookie, loginResp.Data.CsrfToken, `{"name":"account-redis-image-provider","display_name":"Account Redis Image","adapter_type":"openai-compatible","protocol":"openai-compatible","status":"active","capabilities":{"image_generations":true}}`)
	modelResp := mustCreateModel(t, handler, sessionCookie, loginResp.Data.CsrfToken, `{"canonical_name":"account-redis-image-model","display_name":"Account Redis Image Model","status":"active","capabilities":[{"key":"image_generations","level":"required","status":"stable","version":"v1"}]}`)
...
func mustCreateNativeGeminiGatewayTargetWithNames(t *testing.T, handler http.Handler, sessionCookie *http.Cookie, csrfToken, upstreamURL, providerName, modelName, upstreamModelName, accountName string) (apiopenapi.ProviderResponse, apiopenapi.ModelResponse, apiopenapi.ProviderAccountResponse) {
	t.Helper()
	providerResp := mustCreateProvider(t, handler, sessionCookie, csrfToken, `{"name":"`+providerName+`","display_name":"Native Gemini Route Provider","adapter_type":"gemini-compatible","protocol":"gemini-compatible","status":"active","capabilities":{"token_counting":true}}`)
	modelResp := mustCreateModel(t, handler, sessionCookie, csrfToken, `{"canonical_name":"`+modelName+`","display_name":"Native Gemini Route Model","status":"active","capabilities":[{"key":"streaming","level":"required","status":"stable","version":"v1"},{"key":"token_counting","level":"required","status":"stable","version":"v1"}]}`)
	mustCreateMapping(t, handler, sessionCookie, csrfToken, string(modelResp.Data.Id), `{"provider_id":"`+string(providerResp.Data.Id)+`","upstream_model_name":"`+upstreamModelName+`","status":"active"}`)
	accountResp := mustCreateAccount(t, handler, sessionCookie, csrfToken, `{"provider_id":"`+string(providerResp.Data.Id)+`","name":"`+accountName+`","runtime_class":"api_key","credential":{"api_key":"native-gemini-secret"},"metadata":{"base_url":"`+upstreamURL+`/v1beta"},"status":"active"}`)
	return providerResp, modelResp, accountResp
}
...
func TestAdminOpsAlertRulesControlPlane(t *testing.T) {
	...
	for _, name := range expectedBaselineNames {

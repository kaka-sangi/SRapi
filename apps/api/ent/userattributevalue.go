...
func (*UserAttributeValue) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case userattributevalue.FieldID, userattributevalue.FieldUserID, userattributevalue.FieldDefinitionID:
...
func (_m *UserAttributeValue) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case userattributevalue.FieldDefinitionID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field definition_id", values[i])
			} else if value.Valid {
				_m.DefinitionID = int(value.Int64)
			}
		case userattributevalue.FieldValue:
